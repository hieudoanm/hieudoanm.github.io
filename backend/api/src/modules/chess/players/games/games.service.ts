...
export class GamesService {
  ...
  public async getGames(
    ...
    const [total = 0, games = []] = await this.prismaService.$transaction([
      this.prismaService.chessGame.count({ where, take: limit, skip: offset }),
      this.prismaService.chessGame.findMany({
        where,
        take: limit,
        skip: offset,
  ...
  public async getGame(
    ...
    const game: ChessGameDto =
      await this.prismaService.chessGame.findFirstOrThrow({
        where,
      });
    return game;
  }
  ...
  private async upsertGames(games: ChessGameDto[]): Promise<ChessGameDto[]> {
    const upsertTransactions = games.map((game) => {
      ...
      return this.prismaService.chessGame.upsert(upsertArguments);
    });
    return this.prismaService.$transaction(upsertTransactions);
  }

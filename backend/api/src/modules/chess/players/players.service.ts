...
export class PlayerService {
  ...
  async getPlayers({
    ...
    const players = await this.prismaChessClient.chessPlayer.findMany({
      take: limit,
      skip: offset,
      include: { stats: true },
    });
  ...
  async getPlayer(username: string): Promise<ChessPlayerDto> {
    const where: Prisma.ChessPlayerWhereInput = { username };
    const player = await this.prismaChessClient.chessPlayer.findFirst({
      where,
      include: { stats: true },
    });
  ...
  private async syncProfile(username: string) {
    ...
    return this.prismaChessClient.chessPlayer.upsert({
      create: player,
      update: player,
      where: { id: player.id },
      include: { stats: true },
  ...
  private async syncStats(username: string, id: number): Promise<number> {
    ...
    for (const timeClassStats of playerStats) {
      await this.prismaChessClient.chessStats.upsert({
  ...
  async syncPlayer(username: string) {

from chess import Board, Move, parse_square, PIECE_NAMES, Piece
...
DEVELOPMENT: str = get_env("DEVELOPMENT", "true")
...
def get_piece_name(board: Board, uci: str):
    piece: Piece = board.piece_at(parse_square(uci[-2:]))
...
def get_move_centipawn(
    ...
    if mate is None:
        ...
        centipawn: int = (
            MAX_CENTIPAWN if turn == "white" else -1 * MAX_CENTIPAWN
        )
    return centipawn
...
def get_moves(game: Game) -> list[dict]:
    ...
    for index, move_with_evaluation in enumerate(moves_with_evaluation):
        ...
        move_quality: dict[str, str] = get_move_quality(
...
async def analyse_pgn(pgn_request_body: PgnRequestBody) -> dict:
    ...
    white_accuracies: list[float] = list(
    ...
    black_accuracies: list[float] = list(

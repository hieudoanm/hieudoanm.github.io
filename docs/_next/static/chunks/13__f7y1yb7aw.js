...
}`,rN=e=>{let r=tk(e.inputs[0].dataType);e.compute(rb(e.inputs[0],"Erf",e=>`erf_vf32(${e})`,rM(r)))},rD=e=>{e.compute(rb(e.inputs[0],"Exp","exp"))},rP=e=>{e.compute(rb(e.inputs[0],"Floor","floor"))},rU=e=>{let r=tk(e.inputs[0].dataType);e.compute(rb(e.inputs[0],"Gelu",e=>`0.5 * ${e} * (1.0 + erf_vf32(${e} * 0.7071067811865475))`,rM(r)))},rq=(e,r)=>{let i=tk(e.inputs[0].dataType);e.compute(rb(e.inputs[0],"LeakyRelu",e=>`select(leaky_relu_alpha_ * ${e}, ${e}, ${e} >= vec4<${i}>(0.0))`,`const leaky_relu_alpha_ = ${i}(${r.alpha});`,r.cacheKey))},rW=e=>{e.compute(rb(e.inputs[0],"Not",e=>`!${e}`))},rL=e=>{e.compute(rb(e.inputs[0],"Neg",e=>`-${e}`))},rV=e=>{e.compute(rb(e.inputs[0],"Reciprocal",e=>`1.0/${e}`))},rG=e=>{let r=tk(e.inputs[0].dataType);e.compute(rb(e.inputs[0],"Relu",e=>`select(vec4<${r}>(0.0), ${e}, ${e} > vec4<${r}>(0.0))`))},rH=e=>{e.compute(rb(e.inputs[0],"Sigmoid",e=>`(1.0 / (1.0 + exp(-${e})))`))},rF=e=>tv(e),rj=(e,r)=>{let i=tk(e.inputs[0].dataType);e.compute(rb(e.inputs[0],"HardSigmoid",e=>`max(vec4<${i}>(0.0), min(vec4<${i}>(1.0), ${r.alpha} * ${e} + vec4<${i}>(${r.beta})))`,void 0,r.cacheKey))},rK=e=>{e.compute(rb(e.inputs[0],"Sin","sin"))},rZ=e=>{e.compute(rb(e.inputs[0],"Sinh","sinh"))},rQ=e=>{e.compute(rb(e.inputs[0],"Sqrt","sqrt"))},rX=e=>{e.compute(rb(e.inputs[0],"Tan","tan"))},rY=e=>`sign(${e}) * (1 - exp(-2 * abs(${e}))) / (1 + exp(-2 * abs(${e})))`,rJ=e=>{e.compute(rb(e.inputs[0],"Tanh",rY))},r0=(e="f32")=>`
...
`}),nU=W(()=>{nf(),ny(),nS(),nN(),i$=(e,r,i,a,n)=>{let s=a-i;return`
...
`}return e})()}
  ...
  `}}}}),nq=W(()=>{nf(),ny(),nS(),nN(),nU(),nD(),iw=(e,r,i="f32",a,n=!1,s=32,o=!1,u=32)=>{let l,d,p,c,h=r[1]*e[1],f=r[0]*e[0],m=n?h:s,g=n?s:h,y=m/r[0],_=s/r[1];if(!((n&&4===y&&4===e[1]||!n&&(3===y||4===y))&&m%r[0]==0&&s%r[1]==0&&4===e[0]))throw Error(`If transposeA ${n} is true, innerElementSize ${y} and workPerThread[1] ${e[1]} must be 4.
...
`)}
  ...
  ${r.mainStart()}
    ...
    ${_?"inv_std_output[global_idx] = inv_std_dev":""};
        ...
        }`}})):e.compute((n=e.inputs,s=r,A=(C=n[0].dims).length,O=C[A-2],R=s.k,B=s.n,M=C.slice(0,A-2),N=e3.size(M),D=n[1].dims[2]/4,P=n[0].dataType,U=tI(s.k),q=tI(D),W=tI(B),L=M.concat([O,B]),V=O>1&&B/W%2==0?2:1,G=e3.size(L)/W/V,H=[],F=[N,O,R/U],(j=e3.convertShape(n[1].dims).slice()).splice(-1,1,D/q),H.push(...tT(F)),H.push(...tT(j)),H.push(...tT(n[2].dims)),4===n.length&&H.push(...tT(e3.convertShape(n[3].dims))),K=[N,O,B/W],H.push(...tT(K)),{name:"MatMulNBits",shaderCache:{hint:`${s.blockSize};${s.bits};${U};${q};${W};${V};64`,inputDependencies:Array(n.length).fill("rank")},getRunData:()=>({outputs:[{dims:L,dataType:P}],dispatchGroup:{x:G},programUniforms:H}),getShaderSource:e=>{let r=F.length,i=tR("a",n[0].dataType,r,U),a=tR("b",12,j.length,q),o=tR("scales",n[2].dataType,n[2].dims.length),u=[i,a,o],l=4===n.length?tR("zero_points",12,n[3].dims.length):void 0;l&&u.push(l);let d=K.length,p=tB("output",n[0].dataType,d,W),c=tS(n[0].dataType),h=(()=>{switch(U){case 1:return`array<${c}, 8>`;case 2:return`mat4x2<${c}>`;case 4:return`mat2x4<${c}>`;default:throw Error(`${U}-component is not supported.`)}})(),f=Math.floor(32/s.bits),m=Math.floor(f/8);return`

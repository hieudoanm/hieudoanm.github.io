...
mod tests {
    ...
    async fn test_webhook_crud() {
        ...
        let (status, value) = request(&app, "POST", "/api/webhooks", Some(&token), Some(body)).await;
        assert_eq!(status, StatusCode::OK);
        ...
        let (status, value) = request(&app, "GET", &format!("/api/webhooks/{hook_id}"), Some(&token), None).await;
        assert_eq!(status, StatusCode::OK);
        assert_eq!(value["name"], "my hook");

        let body = serde_json::json!({"name": "updated hook", "is_active": false});
        let (status, value) = request(&app, "PATCH", &format!("/api/webhooks/{hook_id}"), Some(&token), Some(body)).await;
        assert_eq!(status, StatusCode::OK);
        assert_eq!(value["name"], "updated hook");
        assert!(!value["is_active"].as_bool().unwrap_or(true));

        let (status, _) = request(&app, "DELETE", &format!("/api/webhooks/{hook_id}"), Some(&token), None).await;
        assert_eq!(status, StatusCode::NO_CONTENT);

        let (status, _) = request(&app, "GET", &format!("/api/webhooks/{hook_id}"), Some(&token), None).await;
        assert_eq!(status, StatusCode::NOT_FOUND);
    }
    ...
    async fn test_webhook_validation() {
        ...
        let cases = vec![
            (serde_json::json!({"url": "https://example.com/h", "events": ["record.create"]}), "name is required"),
            (serde_json::json!({"name": "test", "events": ["record.create"]}), "url is required"),
            (serde_json::json!({"name": "test", "url": "https://example.com/h"}), "at least one event is required"),
            (serde_json::json!({"name": "test", "url": "https://example.com/h", "events": ["invalid.event"]}), "unknown event"),
        ];
        for (i, (body, expected)) in cases.iter().enumerate() {
            let (status, value) = request(&app, "POST", "/api/webhooks", Some(&token), Some(body.clone())).await;
            assert_eq!(status, StatusCode::BAD_REQUEST, "case {i}: expected BAD_REQUEST for '{expected}'");
            let err = value["error"].as_str().unwrap_or("");
            assert!(err.contains(expected), "case {i}: expected '{expected}' in '{err}'");
        }
    }
    ...
    async fn test_webhook_logs() {
        ...
        let (status, value) = request(&app, "GET", &format!("/api/webhooks/{hook_id}/logs"), Some(&token), None).await;
        assert_eq!(status, StatusCode::OK);
    ...
    async fn test_webhook_event_validation() {
        ...
        assert!(value["error"].as_str().unwrap_or("").contains("unknown event"));
    }
    ...
    async fn test_webhook_events_dispatch_on_record_create() {
        ...
        let (status, _) = request(&app, "POST", "/api/collections/items/records", Some(&token), Some(rec_body)).await;
        assert_eq!(status, StatusCode::OK);

...
A lightweight Supabase/PocketBase-like backend in Rust with SQLite, featuring 22+ modules.
...
```bash
docker build -t backbone-rust .
docker run -p 8080:8080 -v backbone-data:/data backbone-rust
```
...
- **Database:** SQLite via `rusqlite` + `deadpool` connection pooling (pool size 8)
...
- **Static Files:** Embedded via `rust-embed` (no disk dependency)
- **Middleware:** Rate limiting, content-type validation, request body size limit (10 MB), request logging, CORS

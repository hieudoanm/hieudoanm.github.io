export const toRS = (data: any, rootName = 'Root'): string => {
  ...
  const parseType = (value: any, key?: string): string => {
    ...
    if (Array.isArray(value)) {
      ...
      return types.length === 1 ? `Vec<${types[0]}>` : `Vec<serde_json::Value>`; // fallback for mixed arrays
    }
  ...
  const parseObject = (obj: Record<string, any>, structName: string) => {
    ...
    for (const [key, value] of entries) {
      ...
      // handle snake_case vs reserved keywords

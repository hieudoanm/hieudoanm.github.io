...
const CardChip: FC<{
  ...
  <div
    onClick={onClick}
    className="border-base-300 bg-base-200 hover:border-base-content/30 relative flex h-14 w-10 cursor-pointer items-center justify-center rounded border-2 border-dashed text-center text-xs font-bold transition-colors">
    {card ? (
      <>
        <span className={isRed(card.suit) ? 'text-error' : 'text-base-content'}>
          {card.rank}
          {SUIT_DISPLAY[card.suit]}
        </span>
        <button
          onClick={(e) => {
            e.stopPropagation();
          ...
          className="bg-base-content text-base-100 absolute -top-1.5 -right-1.5 flex h-4 w-4 items-center justify-center rounded-full text-[10px] leading-none">
          ×
        </button>
      </>
    ) : (
...
const MeterBar: FC<{ label: string; pct: number; color: string }> = ({
  label,
  pct,
  color,
}) => (
  <div className="flex items-center gap-2 text-xs">
    <span className="w-8 opacity-50">{label}</span>
  ...
  return (
    <dialog className="modal modal-open" style={{ zIndex: 1001 }}>
      <div className="modal-box w-72 p-4" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="mb-3 text-sm font-bold">Pick a card</p>

        <p className="mb-1 text-xs opacity-50">Suit</p>
        <div className="mb-3 flex gap-2">
          {SUITS.map((s) => (
            <button
              key={s}
              onClick={() => setSuit(s)}
              className={`btn btn-sm flex-1 ${suit === s ? 'btn-neutral' : 'btn-ghost'} ${isRed(s) ? 'text-error' : ''}`}>
              {SUIT_DISPLAY[s]}
            </button>
          ))}
        </div>

        <p className="mb-1 text-xs opacity-50">Rank</p>
        <div className="grid grid-cols-7 gap-1">
          {RANKS.map((r) => {
            const key = suit ? r + suit : null;
            const isUsed = key
              ? used.includes(key) &&
                (current ? cardKey(current) !== key : true)
              : false;
            return (
              <button
                key={r}
                disabled={!suit || isUsed}
                onClick={() => suit && onSelect({ rank: r, suit })}
                className="btn btn-xs btn-ghost disabled:opacity-20">
                {r}
              </button>
            );
          })}
...
export const PokerModal: FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const [picker, setPicker] = useState<{ slot: SlotKey; idx: number } | null>(
    null
  );
  const [results, setResults] = useState<Results | null>(null);
  const [simulating, setSimulating] = useState(false);

  const usedCards = [...hero, ...villain, ...community]
  ...
  const handleSelect = (card: Card) => {
    if (!picker) return;
    setCard(picker.slot, picker.idx, card);
    setPicker(null);
  };

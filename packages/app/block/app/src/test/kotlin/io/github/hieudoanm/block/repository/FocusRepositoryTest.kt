...
class FocusRepositoryTest {
    ...
    fun setup() {
        dao = io.mockk.mockk()
        settingsDataStore = io.mockk.mockk()
    ...
    fun `toggleBlockedApp inserts when blocked`() = runTest {
        ...
        io.mockk.verify { dao.upsertBlockedApp(io.mockk.match {
            it.packageName == "com.example.app" && it.label == "Example" && it.enabled
        }) }
    }
    ...
    fun `toggleBlockedApp removes when not blocked`() = runTest {
        ...
        io.mockk.verify { dao.removeBlockedApp("com.example.app") }
    }
    ...
    fun `setBlockingEnabled delegates to settings`() = runTest {
        ...
        io.mockk.verify { settingsDataStore.setBlockingEnabled(true) }
    }
}

...
type Cell = 'empty' | 'snake' | 'head' | 'food';
...
const NEXT: Record<Dir, (p: Pos) => Pos> = {
  UP: (p) => ({ r: p.r - 1, c: p.c }),
...
export const SnakeModal: FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  return (
    <ModalWrapper onClose={onClose} title="Snake" size="max-w-sm" fullHeight>
      <div
        ref={containerRef}
        tabIndex={0}
        onKeyDown={onKeyDown}
          ...
          <div
            ...
            {grid.flat().map((cell, i) => {
              ...
              return (
                <div
                  key={`${r}-${c}`}
                  className={`border-base-200 aspect-square border ${
                    cell === 'food'
                      ? 'bg-error/70'
                      : cell === 'head'
                        ? 'bg-success'

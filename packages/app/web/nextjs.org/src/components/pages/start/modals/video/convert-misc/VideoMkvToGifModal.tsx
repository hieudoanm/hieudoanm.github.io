...
function encodeGif(frames: ImageData[], delay: number): Blob {
...
function buildPalette(frames: ImageData[], maxColors: number): number[][] {
...
function quantizePixels(frame: ImageData, palette: number[][]): number[] {
  const d = frame.data;
  const pixels: number[] = [];
  for (let i = 0; i < d.length; i += 4) {
    let bestIdx = 0,
...
export const VideoMkvToGifModal: FC<{ onClose: () => void }> = ({
  onClose,
}) => {
  ...
  return (
    <ModalWrapper onClose={onClose} title="MKV to GIF">
      <div className="flex flex-col gap-4">
        <p className="text-sm">Convert MKV videos to animated GIF.</p>
        <input
          type="file"
          accept="video/*"
...
VideoMkvToGifModal.displayName = 'VideoMkvToGifModal';

...
Checklist for shipping Clipper to users: macOS notarization, DMG creation, and
...
- [ ] DMG contains `Clipper.app` and `/Applications` symlink
...
- [ ] Notarize the app:
  ```bash
  xcrun notarytool submit build/Clipper-0.0.1.dmg \
    --apple-id "$APPLE_ID" \
...
- [ ] Staple the notarization:
  ```bash
  xcrun stapler staple build/Clipper-0.0.1.dmg
  ```
- [ ] Verify Gatekeeper: `spctl --assess --type execute build/Clipper.app`

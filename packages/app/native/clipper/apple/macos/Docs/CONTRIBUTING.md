...
Thanks for contributing to **Clipper**, a native macOS clipboard manager.
...
2. **Clone the repo** and navigate to the project:
   ...
   ```bash
   cd packages/app/native/clipper/apple/macos
   ```
...
2. **Flat over deeply nested** — short functions, minimal indentation, guard
   clauses.
3. **Self-documenting identifiers** — `getClipboardHistory()` needs no comment.
...
1. Isolate AppKit code from SwiftUI.
2. Handle clipboard access errors gracefully.
3. Never crash on pasteboard errors — refresh state instead.
...
4. Verify clipboard monitoring works

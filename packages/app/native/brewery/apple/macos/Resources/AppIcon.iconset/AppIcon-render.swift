...
for element in elements {
    ...
    case .rect(let x, let y, let width, let height, let rx, let ry, _):
        // Flip Y so SVG (top-left origin) renders correctly:
        // CoreGraphics places high Y at the top of the image.
        let flippedY = CGFloat(size) - y - height
        let path = CGPath(
            roundedRect: CGRect(x: x, y: flippedY, width: width, height: height),
            cornerWidth: rx,
            cornerHeight: ry,
            transform: nil
        )
    ...
    case .circle(let cx, let cy, let radius, _):
        let flippedY = CGFloat(size) - cy
        context.fillEllipse(
            in: CGRect(x: cx - radius, y: flippedY - radius, width: radius * 2, height: radius * 2)
        )
    }
}

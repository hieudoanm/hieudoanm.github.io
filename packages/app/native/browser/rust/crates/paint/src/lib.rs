...
pub fn build_display_list(boxes: &[LayoutBox], dom: &Dom, styles: &StyleContext) -> DisplayList {
    let mut list = DisplayList::new();
    for box_ in boxes {
        build_box_commands(box_, dom, styles, &mut list);
    }
    list
}

fn build_box_commands(box_: &LayoutBox, dom: &Dom, styles: &StyleContext, list: &mut DisplayList) {
    ...
    for child in &box_.children {
        build_box_commands(child, dom, styles, list);
    }
}
...
mod tests {
    ...
    fn build_empty_display_list() {
        ...
        let list = build_display_list(&[], &dom, &styles);
        assert!(list.is_empty());
    }
    ...
    fn build_display_list_with_element() {
        ...
        let list = build_display_list(&[box_], &dom, &styles);
        assert!(list.is_empty());
    }

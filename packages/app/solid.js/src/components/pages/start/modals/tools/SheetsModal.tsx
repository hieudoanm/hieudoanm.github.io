...
const IcoChevronsUpDown = (props: { className?: string }) => (
  <svg
    class={props.className ?? 'h-3 w-3'}
    viewBox="0 0 16 16"
    fill="none"
    stroke="currentColor"
    stroke-width="1.5">
...
const CellValue = (props: { value: CellVal }) => {
  if (props.value === null)
    return <span class="text-base-content/30 text-xs italic">NULL</span>;
  if (props.value instanceof Uint8Array)
    return (
      <span class="text-warning/70 text-xs italic">
        [BLOB {props.value.length}B]
      </span>
    );
  if (typeof props.value === 'number')
    return <span class="text-primary font-mono">{props.value}</span>;
  const s = String(props.value);
  if (s === '1' || s.toLowerCase() === 'true')
    return <span class="text-success font-mono text-xs">{s}</span>;
  if (s === '0' || s.toLowerCase() === 'false')
    return <span class="text-error font-mono text-xs">{s}</span>;
  return (
...
const SortIcon = (props: { active: boolean; dir: number }) => {
  if (!props.active)
    return (
      <IcoChevronsUpDown class="text-base-content/20 h-3 w-3 opacity-0 transition-opacity group-hover:opacity-100" />
    );
  return props.dir === 1 ? <IcoChevUp /> : <IcoChevDown />;
};
...
const ExportModal = (props: {
  ...
  const content = () =>
    getExportContent(format(), props.tableName, props.columns, props.rows);
  ...
  return (
    <div
      ...
      <div
        ...
        <div class="border-base-300 flex flex-shrink-0 items-center justify-between border-b px-5 py-4">
          <div class="flex items-center gap-3">
            <span class="text-base-content font-bold tracking-tight">
              Export{' '}
              <span class="text-primary font-mono">{props.tableName}</span>
            </span>
            <span class="text-base-content/30 font-mono text-xs">
              {formatNumber(props.rows.length)} rows · {props.columns.length}{' '}
              cols
            </span>
          </div>
          <button
        ...
        <div class="flex flex-shrink-0 gap-1 px-5 pt-4">
          {EXPORT_FORMATS.map((f) => (
            <button
              key={f.value}
              onClick={() => setFormat(f.value)}
              class={`btn btn-sm rounded-lg transition-all ${format() === f.value ? 'btn-primary' : 'btn-ghost text-base-content/50 hover:text-base-content'}`}>
              {f.label}
            </button>
          ))}
        </div>
        <div class="min-h-0 flex-1 overflow-auto px-5 py-3">
          <textarea
            readOnly
            class="bg-base-200 border-base-300 h-64 w-full resize-none rounded-lg border p-3 font-mono text-xs leading-relaxed focus:outline-none"
            value={content()}
          />
        </div>
        <div class="border-base-300 flex flex-shrink-0 items-center gap-2 border-t px-5 py-4">
          <button
          ...
          <button
            ...
            onClick={() => {
              ...
              a.href = url;
              a.download = `${props.tableName}.${fmt().ext}`;
              a.click();
...
export const SheetsModal = (props: { onClose: () => void }) => {
  const [sqlJs, setSqlJs] = createSignal<SqlJsStatic | null>(null);
  const [dbInstance, setDbInstance] = createSignal<SqlDatabase | null>(null);
  const [dbFileName, setDbFileName] = createSignal<string | null>(null);
  const [tables, setTables] = createSignal<TableMeta[]>([]);
  const [activeTable, setActiveTable] = createSignal<string | null>(null);
  const [queryResult, setQueryResult] = createSignal<QueryResult>({
    columns: [],
    rows: [],
  });
  const [filteredRows, setFilteredRows] = createSignal<QueryResult['rows']>([]);
  ...
  const [status, setStatus] = createSignal('Ready · No database loaded');
  const [isDragging, setIsDragging] = createSignal(false);
  const [opfsFiles, setOpfsFiles] = createSignal<string[]>([]);
  const [showExport, setShowExport] = createSignal(false);

  let fileInputRef: HTMLInputElement | undefined;
  let dbRef: SqlDatabase | null = null;
  ...
  const openDb = async (buffer: Uint8Array, filename: string) => {
    setLoading(true);
  ...
  return (
    <ModalWrapper
      ...
      {/* Drag overlay */}
      {isDragging() && (
        <div class="bg-base-100/80 pointer-events-none fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm">
          <div class="border-primary rounded-2xl border-2 border-dashed p-16 text-center">
            <p class="text-primary text-xl font-semibold tracking-widest uppercase">
              Drop .db file
            </p>
          </div>
        </div>
      ...
      <div
        ...
        onDrop={handleDrop}>
        {/* Topbar */}
        <header class="bg-base-200 border-base-300 flex flex-shrink-0 items-center gap-3 border-b px-4 py-2">
          <button
          ...
          {dbFileName() && (
            <div class="badge badge-outline badge-primary gap-2 px-3 py-3 font-mono text-xs">
              <span class="bg-success inline-block h-2 w-2 animate-pulse rounded-full" />
              {dbFileName()}
            </div>
          )}
          <div class="ml-auto flex items-center gap-2">
            {opfsFiles().length > 0 && (
              <div class="dropdown dropdown-end">
                ...
                <ul
                  tabIndex={0}
                  class="dropdown-content menu bg-base-200 border-base-300 z-50 mt-2 w-52 rounded-xl border p-2 shadow-xl">
                  {opfsFiles().map((f) => (
                    <li key={f}>
                      <a
        ...
        <div class="flex flex-1 overflow-hidden">
          {/* Sidebar */}
          <aside class="bg-base-200 border-base-300 flex w-56 flex-shrink-0 flex-col overflow-hidden border-r">
            <div class="px-3 pt-3 pb-1">
              <p class="text-base-content/30 text-[10px] font-semibold tracking-widest uppercase">
                Tables
              </p>
            </div>
            <div class="flex-1 space-y-0.5 overflow-y-auto px-2 pb-2">
              {tables().length === 0 && (
                <p class="text-base-content/30 px-2 py-3 text-xs italic">
                  No tables
                </p>
              )}
              {tables().map((t) => (
                <button
                  ...
                  class={`group flex w-full items-center gap-2 rounded-lg px-2 py-2 text-left transition-all ${activeTable() === t.name ? 'bg-primary/10 text-primary border-primary/20 border' : 'text-base-content/60 hover:text-base-content hover:bg-base-300 border border-transparent'}`}>
                  <span
                    class={`flex-shrink-0 ${activeTable() === t.name ? 'text-primary' : 'text-base-content/30'}`}>
                    <IcoTable />
                  </span>
                  <span class="flex-1 truncate text-xs font-medium">
                    {t.name}
                  </span>
                  <span class="text-base-content/30 font-mono text-[10px] tabular-nums">
            ...
            {opfsFiles().length > 0 && (
              <>
                <div class="border-base-300 border-t px-3 pt-3 pb-1">
                  <p class="text-base-content/30 text-[10px] font-semibold tracking-widest uppercase">
                    Saved (OPFS)
                  </p>
                </div>
                <div class="space-y-0.5 px-2 pb-2">
                  {opfsFiles().map((f) => (
                    <button
                      key={f}
                      onClick={() => handleLoadOpfs(f)}
                      class="text-base-content/50 hover:text-base-content hover:bg-base-300 flex w-full items-center gap-2 rounded-lg px-2 py-2 text-left transition-all">
                      <span class="text-base-content/30 flex-shrink-0">
                        <IcoDatabase />
                      </span>
                      <span class="truncate font-mono text-xs">{f}</span>
                    </button>
                  ))}
                </div>
              </>
          ...
          {/* Main */}
          <main class="bg-base-100 flex flex-1 flex-col overflow-hidden">
            {!dbInstance() && (
              <div class="flex flex-1 flex-col items-center justify-center gap-6 p-8 text-center">
                {loading() ? (
                  <div class="flex flex-col items-center gap-4">
                    ...
                    <p class="text-base-content/50 font-mono text-sm">
                      {loadingMsg()}
                    </p>
                  </div>
                ) : (
                  <>
                    <div class="text-base-content/10">
                      <svg
                        class="h-20 w-20"
                        viewBox="0 0 48 48"
                        fill="none"
                    ...
                    <div>
                      <h1 class="text-base-content/80 mb-2 text-2xl font-bold tracking-tight">
                        Sheets
                      </h1>
                      <p class="text-base-content/40 max-w-xs text-sm leading-relaxed">
                        Open a{' '}
                        <code class="text-primary bg-primary/10 rounded px-1 text-xs">
                          .db
                        </code>{' '}
                        file or drop it anywhere to browse your SQLite database.
                        Persisted via <span class="text-primary">OPFS</span> —
                        no server needed.
                      </p>
                    </div>
            ...
            {dbInstance() && (
              <>
                <div class="border-base-300 bg-base-200/50 flex flex-shrink-0 items-center gap-3 border-b px-4 py-2.5">
                  {activeTable() ? (
                    <>
                      <span class="text-base-content font-mono text-sm font-semibold">
                        {activeTable()}
                      </span>
                      <div class="badge badge-ghost badge-sm text-base-content/40 font-mono">
                  ...
                  <div class="ml-auto flex items-center gap-2">
                    ...
                    <label class="input input-bordered input-sm bg-base-100 flex w-48 items-center gap-2">
                      <span class="text-base-content/30">
                        <IcoSearch />
                      </span>
                      <input
                        type="text"
                        class="grow bg-transparent font-mono text-xs"
                ...
                {activeTable() && queryResult().columns.length > 0 ? (
                  <div class="flex-1 overflow-auto">
                    <table class="table-xs table-pin-rows table w-full">
                      <thead>
                        <tr class="bg-base-200">
                          <th class="text-base-content/20 w-10 text-center font-mono font-normal">
                            #
                          </th>
                          {queryResult().columns.map((col, i) => (
                            <th
                              key={col}
                              class="hover:text-primary group cursor-pointer whitespace-nowrap transition-colors select-none"
                      ...
                      <tbody>
                        {pageRows().map((row, ri) => (
                          <tr
                            key={ri}
                            class="hover:bg-base-200/40 border-base-300/40 border-b">
                            <td class="text-base-content/20 text-center font-mono text-[10px] tabular-nums">
                              {page() * PAGE_SIZE + ri + 1}
                            </td>
                            {row.map((cell, ci) => (
                              <td key={ci} class="max-w-[200px] truncate">
                  ...
                  <div class="text-base-content/30 flex flex-1 items-center justify-center text-sm italic">
                ...
                {activeTable() && filteredRows().length > PAGE_SIZE && (
                  <div class="border-base-300 bg-base-200/30 flex flex-shrink-0 items-center gap-3 border-t px-4 py-2">
                    <button
                    ...
                    <span class="text-base-content/40 font-mono text-xs tabular-nums">
                    ...
                    <span class="text-base-content/30 ml-auto font-mono text-xs">
        ...
        {/* Statusbar */}
        <footer class="bg-base-200 border-base-300 flex flex-shrink-0 items-center gap-3 border-t px-4 py-1.5">
          {dbInstance() && (
            <span class="bg-success inline-block h-2 w-2 flex-shrink-0 animate-pulse rounded-full" />
          )}
          <span class="text-base-content/40 truncate font-mono text-[11px]">
            {status()}
          </span>
          <span class="text-base-content/20 ml-auto font-mono text-[11px]">
            SQLite WASM · OPFS
          </span>
        </footer>
      </div>

...
export const MorseModal = ({ onClose }: { onClose: () => void }) => {
  ...
  const uniqueChars = () =>
    Array.from(new Set(input().toLowerCase().split(''))).filter(
      (ch) => morse[ch]
    );
    ...
    <ModalWrapper
      ...
      <div class="flex flex-col gap-2">
        <div class="flex items-center justify-between">
          <span class="text-base-content/50 text-xs tracking-widest uppercase">
            Input
          </span>
        ...
        <textarea
          class="textarea textarea-bordered h-24 w-full resize-none font-sans text-sm leading-relaxed"
        ...
        <div class="flex flex-wrap gap-2">
          ...
          {input() && (
            <button
              class="btn btn-ghost btn-xs border-base-300 ml-auto border"
              onClick={() => setInput('')}>
              Clear
            </button>
          )}
      ...
      <div class="flex flex-col gap-2">
        ...
        <div class="bg-base-200 border-base-300 min-h-[64px] overflow-auto rounded-xl border p-4">
          {output() ? (
            <p class="text-primary font-mono text-base leading-loose tracking-widest break-all">
              {output()}
            </p>
          ) : (
            <p class="text-base-content/20 text-sm">
      ...
      <div class="flex flex-wrap gap-2">
        <button
          ...
          {playing() ? (
            <>
              <span class="loading loading-spinner loading-xs" /> Playing…
            </>
          ) : (
            '▶ Play'
          )}
        ...
        <button
          class={`btn btn-sm ${copied() ? 'btn-success' : 'btn-ghost border-base-300 border'}`}
          onClick={copy}
          disabled={!output()}>
          {copied() ? '✓ Copied' : 'Copy'}
        </button>
        <button
          class="btn btn-ghost btn-sm border-base-300 border"
          onClick={download}
          disabled={!input}>
          ↓ .morse
        </button>
        {uniqueChars().length > 0 && (
          <button
            class="btn btn-ghost btn-sm border-base-300 ml-auto border"
        ...
        <div class="border-base-300 overflow-hidden rounded-xl border">
          <table class="table-sm table w-full text-sm">
            <thead class="bg-base-200">
              <tr>
                <th class="text-base-content/40 font-medium">Char</th>
                <th class="text-base-content/40 font-medium">Code</th>
                <th class="text-base-content/40 hidden font-medium sm:table-cell">
                  Pattern
                </th>
              </tr>
            </thead>
            <tbody>
              {uniqueChars().map((ch) => {
                ...
                return (
                  <tr key={ch} class="border-base-300 border-t">
                    <td class="font-mono font-medium uppercase">
                      {ch === ' ' ? '␣' : ch}
                    </td>
                    <td class="text-primary font-mono tracking-widest">
                      {entry.code}
                    </td>
                    <td class="text-base-content/40 hidden font-mono text-xs sm:table-cell">
                      {entry.pattern}
                    </td>
                  </tr>
                );

...
export const GitHubSocialPreviewModal = ({
  onClose,
}: {
  onClose: () => void;
}) => {
  ...
  createEffect(() => {
    if (repo() && canvasRef) {
      renderPreview(canvasRef, repo()).catch(console.error);
    }
  });
  ...
  const download = () => {
    if (!canvasRef || !repo()) return;
    const a = document.createElement('a');
    a.href = canvasRef.toDataURL('image/png');
  ...
  return (
    <ModalWrapper
      ...
      {repo() && (
        <>
          <div class="bg-base-200 mb-3 overflow-hidden rounded-xl">
            <canvas
              ref={(el) => (canvasRef = el)}
              class="w-full"
          ...
          {/* Meta */}
          <div class="mb-4 flex items-center gap-3 text-xs opacity-50">
          ...
          <div class="flex gap-2">
            <button class="btn btn-primary btn-sm flex-1" onClick={download}>
              ⬇ Download PNG
            </button>
      ...
      {!repo() && !loading() && !error() && (
        <p class="py-6 text-center text-xs opacity-30">

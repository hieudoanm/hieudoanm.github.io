...
export const Versus: FC<TemplateProps> = ({ data }) => {
  ...
  const features = (data.features as { a: string; b: string }[]) ?? [];
  const rows =
    features.length > 0
      ? features
      : [
    ...
    <div className="flex h-full w-full flex-col">
      ...
      <div className="flex flex-1 flex-col gap-3 px-10 pb-8">
        {rows.map((row, i) => (
          <div
            key={i}
            className="rounded-box bg-accent/5 flex items-center justify-between gap-4 px-5 py-3">

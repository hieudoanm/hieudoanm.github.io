    ...
    <div className="bg-base-100 flex h-full w-full flex-col p-8">
      ...
      <div className="flex flex-1 gap-3">
        {categories.map((cat, i) => (
          <div key={i} className="bg-base-200 rounded-box flex-1 p-3">
            ...
            <div className="mt-2 flex flex-col gap-1.5">
              {cat.items.map((item, j) => (
                <div key={j} className="flex items-center gap-2">
                  <span className="bg-accent/20 text-accent flex h-3.5 w-3.5 flex-shrink-0 items-center justify-center rounded-sm text-[7px] font-bold">
                    ✓
                  </span>
                  <span className="text-base-content text-[10px]">{item}</span>
                </div>
              ))}
            </div>
          </div>

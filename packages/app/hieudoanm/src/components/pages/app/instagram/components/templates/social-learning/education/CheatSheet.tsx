  ...
  return (
    <div className="bg-base-100 flex h-full w-full flex-col p-6">
      <div className="mb-3 text-center">
        ...
        <div className="text-base-content mt-1 text-sm font-bold">{title}</div>
      </div>
      <div className="flex flex-1 flex-col gap-1">
        {items.map((item, i) => (
          <div
            key={i}
            className="border-base-300 flex items-stretch gap-2 rounded border">
            <div className="bg-primary flex w-24 shrink-0 items-center justify-center rounded-l text-[9px] font-bold text-white">
              {item.label}
            </div>
            <div className="text-base-content flex flex-1 items-center px-2 font-mono text-[10px]">
              {item.content}
            </div>
          </div>
        ))}

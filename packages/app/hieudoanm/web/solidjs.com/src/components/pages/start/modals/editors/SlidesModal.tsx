...
const TextBlock = ({
  ...
  <div class={className}>
    <pre class="m-0 inline pb-8 break-words whitespace-pre-wrap">{text}</pre>
  </div>
);

const CenterBlock = ({ block }: { block: SlideBlock }) => {
  switch (block.type) {
    case 'title':
      return (
        <TextBlock
    ...
    case 'pricing-plan':
      return (
        <div class="mt-8 flex w-full max-w-sm flex-col items-center">
          <div class="text-base-content mb-8 text-xl font-bold">
            <TextBlock text={block.name} />
          </div>
          <div class="text-primary mb-8 text-6xl font-extrabold">
...
const SlidePreviewComponent = ({
  ...
  <div class="group bg-base-100 border-primary-content relative mx-auto aspect-video h-[720px] w-[1280px] cursor-default border p-14 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl">
    <div class="pointer-events-none absolute inset-0 bg-gradient-to-br from-purple-500 to-transparent opacity-0 transition-opacity group-hover:opacity-20" />
    <div class="flex items-start justify-between">
      {slide.kicker && (
        <div class="text-secondary mb-8 text-lg font-semibold uppercase">
          <TextBlock text={slide.kicker} />
        </div>
      )}
      <div class="text-base-content font-mono text-lg">
      ...
      {slide.blocks.map((block, i) => {
        switch (block.type) {
          ...
          case 'center': {
            ...
            return (
              <div class="-mt-16 flex h-full flex-col items-center justify-center text-center">
                ...
                {pricingPlans.length > 0 && (
                  <div class="mt-12 flex items-center justify-center">
                    {pricingPlans.map((plan, pi) => (
                      <div key={pi} class="flex items-start">
                        <CenterBlock block={plan} />
                        {pi < pricingPlans.length - 1 && (
                          <span class="text-base-100 mx-12 text-9xl font-light">
                            |
                          </span>
                        )}
                      </div>
          ...
          case 'bullets':
            return (
              <ul key={i} class="space-y-6">
                {block.items.map((item, j) => (
                  <li key={j} class="flex gap-4">
                    ...
                    <div>
                      ...
                      {item.description && (
                        <div class="text-neutral-content text-xl">
...
/* =========================
   Landing Component
========================= */
const LandingComponent = ({ data }: { data: PitchDeck }) => {
  const { title, problems, solutions, product, pricing } = data;
  return (
    <div class="bg-base-100 text-base-content min-h-screen">
      <section class="bg-base-200 py-16 sm:py-20 lg:py-24">
        <div class="mx-auto max-w-4xl px-4 text-center">
          <h1 class="mb-6 text-3xl font-bold sm:text-4xl lg:text-5xl">
            {title?.product ?? ''}
          </h1>
          <p class="mb-4 text-lg opacity-80 sm:text-xl">
            {title?.tagline ?? ''}
          </p>
          <p class="mb-8 opacity-70">Built for {title?.audience ?? ''}</p>
        </div>
      </section>
      <section class="py-16 sm:py-20">
        <div class="mx-auto max-w-6xl px-4">
          {problems?.title && (
            <h2 class="mb-6 text-center text-2xl font-bold sm:text-3xl lg:text-4xl">
              {problems.title}
            </h2>
          )}
          {problems?.subtitle && (
            <p class="mb-12 text-center text-base opacity-70 sm:text-lg">
              {problems.subtitle}
            </p>
          )}
          <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {(problems?.items ?? []).map((p, i) => (
              <div
                key={i}
                class="card border-base-300 bg-base-200 border shadow-xl">
                <div class="card-body">
                  <div class="mb-2 text-3xl">{p.emoji}</div>
        ...
        <div class="mx-auto max-w-5xl px-4">
          ...
          <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {(solutions?.items ?? []).map((s, i) => (
              <div
                key={i}
                class="card border-base-300 bg-base-100 border text-center shadow-xl">
                <div class="card-body">
                  <div class="mb-4 text-4xl">{s.emoji}</div>
                  <span class="badge badge-primary mx-auto mb-2">
                    Step {s.step}
                  </span>
                  <h3 class="text-lg font-semibold sm:text-xl">{s.title}</h3>
                  <p class="opacity-80">{s.description}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div class="mx-auto max-w-6xl px-4">
          ...
          <div class="mx-auto grid max-w-4xl gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {(pricing?.plans ?? []).map((plan, i) => (
              <div key={i} class="card bg-base-100 text-center shadow-lg">
                <div class="card-body">
                  <h3 class="text-lg font-semibold sm:text-xl">{plan.name}</h3>
                  <p class="my-4 text-3xl font-bold sm:text-4xl">
                    {formatCurrency(plan?.amount ?? 0, pricing?.currency ?? '')}
                  </p>
                  <p class="mb-2 text-sm opacity-60">{plan.frequency}</p>
                  <p class="mb-6 opacity-80">{plan.description}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <footer class="footer footer-center bg-base-300 text-base-content p-6">
        <aside>
          <p class="font-semibold">{title?.product}</p>
...
const Toast = ({
  ...
  <div class="toast toast-bottom toast-end z-50 space-y-2">
    {toasts.map((toast) => (
      <div
        key={toast.id}
        onClick={() => onDismiss(toast.id)}
        class={`alert animate-toast-in cursor-pointer transition-all duration-300 ease-out ${
          toast.type === 'success'
            ? 'alert-success'
            : toast.type === 'error'
              ? 'alert-error'
...
/* =========================
   Slides Modal
========================= */
export const SlidesModal = ({ onClose }: { onClose: () => void }) => {
  ...
  const [input, setInput] = createSignal(getInitialInput());
  const [showInput, setShowInput] = createSignal(true);
  const [tab, setTab] = createSignal<'pitch' | 'landing'>('pitch');
  ...
  return (
    <ModalWrapper
      ...
      <div class="divide-base-300 flex min-h-0 flex-1 divide-x overflow-hidden">
        {/* Toolbar */}
        <div class="flex flex-col items-center gap-2 p-2">
          <div
            ...
            <button
              class={`btn btn-xs rounded ${showInput() ? 'btn-accent' : 'bg-base-300'}`}
              onClick={() => setShowInput((v) => !v)}>
              📝
            </button>
          </div>
          <div class="tooltip tooltip-right" data-tip="Export PDF">
            <button
          ...
          <div class="tooltip tooltip-right" data-tip="Copy Shareable Link">
            <button
              class="btn btn-xs btn-secondary rounded"
              disabled={!isSharable()}
              onClick={shareURL}>
              🔗
            </button>
        ...
        {showInput() && (
          <div class="flex min-w-0 flex-1 overflow-hidden">
            <div class="bg-base-100 border-base-300 h-full w-full overflow-auto border-r">
              <div
                ref={editorRef}
                class="h-full w-full flex-1 overflow-auto text-sm"
              />
            </div>
          </div>
        )}
        ...
        {/* Preview */}
        <div
          class={`flex min-w-0 flex-1 flex-col overflow-hidden ${showInput() ? '' : 'flex-1'}`}>
          <div class="flex h-full flex-col overflow-auto p-4">
            {!parsed().data && parsed().errors.length > 0 && (
              <div class="alert alert-error mb-4">Invalid YAML</div>
            )}
            ...
            {parsed() && (
              <>
                    ...
                    <ul class="list-disc space-y-1 pl-4 text-sm">
                      {parsed().errors.map((e, i) => (
                        <li key={i}>
                          ...
                          {e.hint && (
                            <span class="opacity-70"> — {e.hint}</span>
                          )}
                        </li>
                      ))}
                    </ul>
                ...
                <div class="tabs tabs-box mb-4 w-full">
                  <button
                    class={`tab w-1/2 rounded ${tab() === 'pitch' ? 'tab-active' : ''}`}
                    onClick={() => setTab('pitch')}>
                    Pitch Deck
                  </button>
                  <button
                    class={`tab w-1/2 rounded ${tab() === 'landing' ? 'tab-active' : ''}`}
                ...
                {tab() === 'pitch' && (
                  <div id="pitch-preview" class="flex flex-col gap-8">
                    {slides().map((slide, i) => (
                ...
                {tab() === 'landing' && parsed().data && (
                  <div class="border-primary-content overflow-hidden rounded border shadow-2xl">

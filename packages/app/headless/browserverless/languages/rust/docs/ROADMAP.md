...
> Phased roadmap, tracking browserverless `AGENTS.md` §46. Status as of 2026-08-28.
> Architecture and capability details that phasing references live in
...
| 4     | Headless rendering      | ✅ offscreen PNG; verified against mvp + real URLs (see [ARCHITECTURE](ARCHITECTURE) §Compatibility)                                                                                 |
...
| 7     | Server                  | ⚠️ `serve` subcommand live (health/version/scrape/screenshot over one serialized worker) — see [ARCHITECTURE](ARCHITECTURE) §Server Mode; separate server crate + hardening deferred |
| 8     | Server hardening        | ❌                                                                                                                                                                                   |
| 9     | Browser state           | ❌                                                                                                                                                                                   |
| 10    | Multiple pages/tabs     | ❌ (blocked: multi-page in one context is unreliable — see [ARCHITECTURE](ARCHITECTURE) §Servo Integration Notes §6)                                                                 |
| 11+   | Product features        | ❌                                                                                                                                                                                   |
...
3. **Cleanup** — delete stale engine crates (`css`/`dom`/`html`/`layout`/`network`/`paint`/
   ...
   fix the `Makefile` `screenshot` target (uses `-p cli`). See [CONTRIBUTING](CONTRIBUTING) §Makefile status.

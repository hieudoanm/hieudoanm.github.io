...
func TestWebhookCronjobData(t *testing.T) {
	job := &store.CronJob{ID: "cj1", Name: "test", Schedule: "*/5 * * * *"}
...
func TestExecuteCronJob_Success(t *testing.T) {
	...
	logs, err := store.ListCronJobLogs(db, job.ID)
	if err != nil {
		t.Fatal(err)
	}
	if len(logs) == 0 {
...
func TestStartCronScheduler_ExecutesJob(t *testing.T) {
	...
	id := "scheduler-job-1"
	_, err := store.InsertCronJob(db, id, "scheduler-test", "* * * * *", ts.URL, "GET", "", "", true)
	if err != nil {
		t.Fatal(err)
	}
	...
	logs, lerr := store.ListCronJobLogs(db, id)
	if lerr != nil {
		t.Fatal(lerr)
	}
	if len(logs) == 0 {

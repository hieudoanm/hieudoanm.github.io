...
func newTestServer(t *testing.T) *Server {
	...
	key, err := secrets.GetOrCreateKey(storageDir)
	if err != nil {
		t.Fatal(err)
	}
	...
	return &Server{db: db, dataDir: storageDir, secretsKey: key, cronScheduler: nil, wsHub: wsHub, cache: cacheStore, sseHub: sseHub, logHub: logHub, pubsubHub: pubsubHub}
}
...
func TestLogCreate(t *testing.T) {
	...
	if logEntry.Message != "test log" {
		t.Fatalf("expected 'test log', got %q", logEntry.Message)
	}
	if logEntry.Level != "info" {
		t.Fatalf("expected 'info', got %q", logEntry.Level)
	}
	if logEntry.ID == "" {
...
func TestDecryptSecretInvalidFormat(t *testing.T) {
	_, err := secrets.DecryptSecret([]byte("12345678901234567890123456789012"), "invalid-format")
	if err == nil {
...
func TestDecryptSecretInvalidHex(t *testing.T) {
	_, err := secrets.DecryptSecret([]byte("12345678901234567890123456789012"), "ZZZZ:data")
	if err == nil {
...
func TestGetOrCreateSecretsKeyEnvVar(t *testing.T) {
	...
	key, err := secrets.GetOrCreateKey(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	if len(key) != 32 {
...
func TestDBFunctions_ClosedDB(t *testing.T) {
	...
	tests := []struct {
		...
		{"deleteBucket", func(db *sql.DB) error { _, err := store.DeleteBucket(db, "test"); return err }},
		{"deleteFile", func(db *sql.DB) error { _, err := store.DeleteFile(db, "test"); return err }},
		{"deleteCacheEntry", func(db *sql.DB) error { _, err := store.DeleteCacheEntry(db, "test"); return err }},
		{"listBuckets", func(db *sql.DB) error { _, err := store.ListBuckets(db); return err }},
		{"listSecrets", func(db *sql.DB) error { _, err := store.ListSecrets(db); return err }},
		{"listCronJobs", func(db *sql.DB) error { _, err := store.ListCronJobs(db); return err }},
		{"listCronJobLogs", func(db *sql.DB) error { _, err := store.ListCronJobLogs(db, "test"); return err }},
		{"listWebhooks", func(db *sql.DB) error { _, err := store.ListWebhooks(db); return err }},
		{"listWebhookLogs", func(db *sql.DB) error { _, err := store.ListWebhookLogs(db, "test", 10); return err }},
		{"listWSConnections", func(db *sql.DB) error { _, err := store.ListWSConnections(db); return err }},
		{"listAllWSMessages", func(db *sql.DB) error { _, err := store.ListAllWSMessages(db); return err }},
		{"insertPubSubTopic", func(db *sql.DB) error { _, err := pubsub.InsertTopic(db, "test", "test"); return err }},
	}

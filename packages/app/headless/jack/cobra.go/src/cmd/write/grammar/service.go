...
func runE(args []string, file string) error {
	input, err := readInput(args, file)
	if err != nil {
		return err
	}

	systemPrompt := "Fix the grammar, spelling, and punctuation of the following text. Return only the corrected text without explanations."

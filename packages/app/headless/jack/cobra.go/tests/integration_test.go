...
func TestMain(m *testing.M) {
	...
	binPath = filepath.Join(root, "bin", "jack-test")
...
func runFail(t *testing.T, args ...string) string {
	...
	stdout, stderr, err := run(args...)
	if err == nil {
		t.Fatalf("jack %v expected failure but succeeded\n  stdout: %s", args, strings.TrimSpace(stdout))
	}
...
func TestFileWriteAndRead(t *testing.T) {
	...
	runOK(t, "file", "write", "--file", path, "--content", "hello from jack")

	data, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	}
	if trimmed := strings.TrimSpace(string(data)); trimmed != "hello from jack" {
		t.Errorf("got %q, want %q", trimmed, "hello from jack")
	}
}
...
func TestNoArgsShowsHelp(t *testing.T) {
	stdout, _, _ := run()
	if !strings.Contains(stdout, "Usage:") && !strings.Contains(stdout, "jack") {

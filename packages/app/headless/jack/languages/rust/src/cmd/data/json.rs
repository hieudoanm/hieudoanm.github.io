...
mod tests {
    ...
    #[test]
    fn test_json_query_invalid_array_index() {
        let data = json!({"items": [1]});
        let result = json_query(&data, ".items[abc]");
        assert!(result.is_err());
    }

    #[test]
    fn test_json_query_array_root() {
        let data = json!([10, 20, 30]);
        let result = json_query(&data, "[1]").unwrap();
        assert_eq!(result, json!(20));
    }

    #[test]
    fn test_read_json_input_with_file() {
        let dir = std::env::temp_dir();
        let path = dir.join("_test_json_input.json");
        std::fs::write(&path, b"{\"key\": \"value\"}").unwrap();
        let result = read_json_input(Some(&path.to_string_lossy().to_string())).unwrap();
    ...
    fn test_json_diff_identical() {
        let dir = std::env::temp_dir();
        let path1 = dir.join("_test_diff1.json");
        let path2 = dir.join("_test_diff2.json");
        std::fs::write(&path1, b"{\"a\": 1}").unwrap();
        std::fs::write(&path2, b"{\"a\": 1}").unwrap();
        // identical files produce no diff lines
    ...
    fn test_json_diff_different() {
        let dir = std::env::temp_dir();
        let path1 = dir.join("_test_diff3.json");
        let path2 = dir.join("_test_diff4.json");
        std::fs::write(&path1, b"{\"a\": 1}").unwrap();
        std::fs::write(&path2, b"{\"a\": 2}").unwrap();
        let result = json_diff(
            &path1.to_string_lossy(),
            &path2.to_string_lossy(),
        );
    ...
    fn test_json_merge_override() {
        let dir = std::env::temp_dir();
        let base = dir.join("_test_merge_base.json");
        let patch = dir.join("_test_merge_patch.json");
        std::fs::write(&base, b"{\"a\": 1, \"b\": 2}").unwrap();
        std::fs::write(&patch, b"{\"b\": 3, \"c\": 4}").unwrap();
        let result = json_merge(
    ...
    #[test]
    fn test_json_merge_empty_patch() {
        let dir = std::env::temp_dir();
        let base = dir.join("_test_merge_empty_base.json");
        let patch = dir.join("_test_merge_empty_patch.json");
        std::fs::write(&base, b"{\"a\": 1}").unwrap();

...
actor ChatEngine {
    ...
    init(config: ChatConfig = .default) {
        self.config = config
        self.messages = [ChatMessage(role: "system", content: "You are jack, a helpful CLI assistant.")]
    }
    ...
    func clear() {
        messages = [ChatMessage(role: "system", content: "You are jack, a helpful CLI assistant.")]
    }
}

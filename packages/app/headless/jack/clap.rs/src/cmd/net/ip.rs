...
mod tests {
    ...
    #[test]
    fn test_command_has_dns_subcommand() {
        let cmd = command();
        let sub = cmd.find_subcommand("dns");
        assert!(sub.is_some());
        let dns = sub.unwrap();
        assert!(dns.get_arguments().any(|a| a.get_id() == "domain"));
    }
    ...
    #[test]
    fn test_detect_vpn_partial_match() {
        assert!(!detect_vpn("mycloudfalcon.com"));
        assert!(!detect_vpn("myamzngstore.com"));
        assert!(!detect_vpn("myisp.net"));
    }
}

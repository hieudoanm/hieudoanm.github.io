...
const STORAGE_KEY = 'brainbow-theme';

const getInitialTheme = (): string => {
  if (typeof window === 'undefined') return 'brainbow-light';
  return localStorage.getItem(STORAGE_KEY) || 'brainbow-light';
};
...
export const Header: FC = () => {
  ...
  const toggleTheme = () =>
    setTheme((current) =>
      current === 'brainbow-light' ? 'brainbow-dark' : 'brainbow-light'
    );
      ...
      <div className="mx-auto flex max-w-5xl items-center justify-between">
        ...
        <nav className="flex items-center gap-0.5">
          ...
          <button
            ...
            {theme === 'brainbow-dark' ? <FiSun /> : <FiMoon />}
          </button>
        </nav>
      </div>
    </header>

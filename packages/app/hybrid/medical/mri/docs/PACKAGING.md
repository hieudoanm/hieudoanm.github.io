...
Checklist for shipping MRI to users: macOS notarization, Windows code-signing,
...
- [ ] Smoke-test with representative MRI datasets (DICOM + NIfTI, rotated and
      differently oriented volumes)
- [ ] Regenerate `docs/ROADMAP.md` release notes for the tag

## Auto-update (desktop)

- [ ] Signing key pair is generated with `pnpm tauri signer generate`
- [ ] Public key is set under `plugins.updater.pubkey` in `tauri.conf.json` (the
...
- [ ] `.AppImage` and `.deb` artifacts build cleanly
- [ ] App runs on a clean Ubuntu 22.04 VM without missing system libraries
- [ ] Bundled scientific-tool discovery verified (or documented as external
      dependency)
...
- [ ] Content URI handling verified: dataset import via file picker works
- [ ] Data-safety form declares no data collection

## iOS

- [ ] Apple Developer account + App Store Connect app record created
- [ ] Signing team + provisioning profile set for the `mri` Xcode project under
      `src-tauri/gen/apple/`

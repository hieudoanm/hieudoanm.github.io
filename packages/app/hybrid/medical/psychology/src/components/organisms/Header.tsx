...
const getInitialTheme = (): string => {
  if (typeof window === 'undefined') return 'psychology-light';
  return localStorage.getItem(STORAGE_KEY) || 'psychology-light';
};
...
export const Header: FC = () => {
  ...
  const toggleTheme = () =>
    setTheme((current) =>
      current === 'psychology-light' ? 'psychology-dark' : 'psychology-light'
    );
      ...
      <div className="mx-auto flex max-w-5xl items-center justify-between">
        ...
        <nav className="flex items-center gap-0.5">
          ...
          <button
            ...
            {theme === 'psychology-dark' ? <FiSun /> : <FiMoon />}
          </button>
        </nav>
      </div>
    </header>

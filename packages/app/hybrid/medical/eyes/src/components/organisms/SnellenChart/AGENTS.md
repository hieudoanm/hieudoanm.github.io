...
```text
SnellenChart/
  index.tsx            # Entry — fullscreen chart + navigation
  utils.ts             # SNELLEN_LINES, LETTERS, randomLetters, generateChart
```

## Overview

Fullscreen Snellen visual acuity chart. Ten lines from 20/200 down to 20/10 show
a randomised letter count per line (1 up to 10); letters stay dimmed until the
...
```tsx
// src/app/(app)/snellen/page.tsx — fullscreen chart
```

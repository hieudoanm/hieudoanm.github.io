...
- [ ] Bump `version` in `src-tauri/tauri.conf.json` and `package.json` in sync
      (the release workflow strips versions from asset filenames, so download
      links never change)
...
- [ ] Upload artifacts to the `app-hybrid-eyes-latest` release tag with the
      exact filenames referenced by `/downloads/` (`eyes_amd64.AppImage`,
      `eyes_amd64.deb`, `eyes_aarch64.dmg`)
...
- [ ] DMG artifact is named `eyes_<version>_aarch64.dmg` (Apple Silicon) to
...
- [ ] Artifacts are named `eyes_<version>_amd64.AppImage` /
      `eyes_<version>_amd64.deb` to match the in-app download links

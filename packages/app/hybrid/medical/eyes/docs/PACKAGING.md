...
- [ ] Bump `version` in `src-tauri/tauri.conf.json` and `package.json` in sync
      (download links embed the version: `eyes_0.0.1_*`)
...
- [ ] Upload artifacts to the `app-hybrid-eyes-latest` release tag with the
      exact filenames referenced by `/downloads/` (`eyes_0.0.1_amd64.AppImage`,
      `eyes_0.0.1_amd64.deb`, `eyes_0.0.1_aarch64.dmg`)

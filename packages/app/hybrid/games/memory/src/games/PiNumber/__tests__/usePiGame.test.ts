...
it('onKeyDown with Space prevents default', () => {
  const { result } = renderHook(() => usePiGame(onClose));
  const preventDefault = jest.fn();
  act(() => {
    result.current.onKeyDown({
...
it('onKeyDown with ArrowRight prevents default', () => {
  const { result } = renderHook(() => usePiGame(onClose));
  const preventDefault = jest.fn();
  act(() => {
    result.current.onKeyDown({
...
it('practice mode does not advance beyond last digit', () => {
  const { result } = renderHook(() => usePiGame(onClose));
  const maxIndex = result.current.digits.length - 1;
  for (let i = 0; i < maxIndex + 5; i++) {
    act(() => {
      result.current.handleKey('ArrowRight');
    });
  }
  expect(result.current.index).toBe(maxIndex);
});

it('digits array matches PI constant', () => {
  const { result } = renderHook(() => usePiGame(onClose));
  expect(result.current.digits[0]).toBe('3');
  expect(result.current.digits[1]).toBe('.');
  expect(result.current.digits[2]).toBe('1');
  expect(result.current.digits[3]).toBe('4');
});

it('game mode digit click sends key to handleKey', () => {
  const { result } = renderHook(() => usePiGame(onClose));
  act(() => {
    result.current.switchToGame();
  });

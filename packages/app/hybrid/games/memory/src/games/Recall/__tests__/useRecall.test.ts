...
it('onKeyDown does nothing for Enter in show phase', async () => {
  const { result } = renderHook(() => useRecall(onClose));
  act(() => {
    result.current.start();
  });
...
it('start resets level to 1', async () => {
  const { result } = renderHook(() => useRecall(onClose));
  await act(async () => {
    result.current.start();
  });
...
it('submit updates high streak correctly', async () => {
  const { result } = renderHook(() => useRecall(onClose));
  await act(async () => {
    result.current.start();
  });
  ...
  const num = result.current.number;
  act(() => {
    result.current.setInput(num);
  });
  await act(async () => {
    result.current.submit();
  });

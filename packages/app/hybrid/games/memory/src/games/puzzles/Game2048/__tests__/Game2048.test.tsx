...
describe('Game2048', () => {
  ...
  it('should render correctly', () => {
    const { container } = render(<Game2048 onClose={jest.fn()} />);
    expect(container).toMatchSnapshot();
  });

  it('renders 16 tiles', () => {
    render(<Game2048 onClose={jest.fn()} />);
  ...
  it('Escape key closes modal', () => {
    ...
    render(<Game2048 onClose={onClose} />);
    const container = screen.getByText('Score:').closest('div')?.parentElement;
    if (container) {
  ...
  it('start and stops auto play with the Auto/Stop button', () => {
    jest.useFakeTimers();
    render(<Game2048 onClose={jest.fn()} />);

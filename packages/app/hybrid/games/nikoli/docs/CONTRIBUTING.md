...
Thanks for contributing to **Nikoli**, a hybrid app that ships as a web app
...
3. **Run this app**:
   ...
   ```bash
   pnpm dev --filter=@hieudoanm.github.io/nikoli
   ```
...
| Task       | Command                                              |
| ---------- | ---------------------------------------------------- |
| Dev server | `pnpm dev --filter=@hieudoanm.github.io/nikoli`      |
| Build      | `pnpm build --filter=@hieudoanm.github.io/nikoli`    |
| Lint       | `pnpm lint --filter=@hieudoanm.github.io/nikoli`     |
| Format     | `pnpm format --filter=@hieudoanm.github.io/nikoli`   |
| Unit tests | `pnpm test --filter=@hieudoanm.github.io/nikoli`     |
| E2E tests  | `pnpm test:e2e --filter=@hieudoanm.github.io/nikoli` |
...
3. Dark theme is the default (`className="dark"` on `<html>`).
4. `prettier-plugin-tailwindcss` sorts classes — keep class order consistent.
5. Use `react-icons` Feather (`Fi`) set for icons.

### Game Architecture

Each game module must follow this 4-file pattern:

- `types.ts` — bilingual `GAME_NAME` constant + domain types. No UI imports.
- `utils.ts` — pure functions for generation, validation, solving. No UI
  imports.
- `use[Game].ts` — custom hook for game state, undo, auto-solve. No UI imports.
- `index.tsx` — React component rendering grid + controls.
...
1. `pnpm lint --filter=@hieudoanm.github.io/nikoli`
2. `pnpm format --filter=@hieudoanm.github.io/nikoli`
3. `pnpm test --filter=@hieudoanm.github.io/nikoli`
4. `pnpm test:e2e --filter=@hieudoanm.github.io/nikoli`
5. `pnpm build --filter=@hieudoanm.github.io/nikoli`

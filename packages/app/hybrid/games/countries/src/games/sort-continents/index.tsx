...
export const ContinentsSort: FC = () => {
  ...
  const onBucketDrop =
    (region: string) => (event: DragEvent<HTMLButtonElement>) => {
      event.preventDefault();
      drop(region as never);
    };

  return (
    <div className="mx-auto flex w-full flex-col gap-3 p-4 sm:px-6 lg:px-10">
        ...
        {REGIONS.map((region) => {
          ...
          return (
            <button
              ...
              className={`flex min-h-[120px] cursor-pointer flex-col rounded-xl border-2 border-dashed p-2 text-left transition-colors ${color.border} ${color.bg}`}>
              <span

# Menu — Packaging Checklist

Checklist for shipping Menu to users: macOS notarization, Windows code-signing,
Android/iOS store builds, and the Tauri updater pipeline.

App: `menu` (bundle identifier `io.github.hieudoanm.menu`, version 0.0.1).
...
- [ ] Endpoint to publish to:
      `https://github.com/hieudoanm/hieudoanm.github.io/releases/download/app-hybrid-business-menu-latest/latest.json`
...
- [ ] Signing team + provisioning profile set for the `menu` Xcode project under
      `src-tauri/gen/apple/`
...
- [ ] Verify QR codes encode a full absolute URL (including base path) so a
      phone camera opens the right menu
- [ ] Confirm guest menu renders from a clean browser (no local data) using only
      the `?d=` payload
- [ ] Confirm the shared link works when the host app runs under a CDN base path

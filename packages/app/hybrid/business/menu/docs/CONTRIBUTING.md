...
Thanks for contributing to **Menu**, a hybrid app that ships as a web app
...
3. **Run this app**:
   ...
   ```bash
   pnpm dev --filter=@hieudoanm.github.io/menu
   ```
...
5. Break tests into small per-file suites — one `*.test.ts` / `*.test.tsx` per
   ...
   directory; never merge multiple units into one file. Pages in route groups
   (`(info)`) colocate `__tests__/page.test.tsx` in the same folder. Use
...
1. `pnpm lint --filter=@hieudoanm.github.io/menu`
2. `pnpm format --filter=@hieudoanm.github.io/menu`
3. `pnpm test --filter=@hieudoanm.github.io/menu`
4. `pnpm test:e2e --filter=@hieudoanm.github.io/menu`
5. `pnpm build --filter=@hieudoanm.github.io/menu`

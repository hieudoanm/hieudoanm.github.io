...
describe('storage', () => {
  ...
  it('lists keys with pos_ prefix', () => {
    storage.set('a', 1);
    storage.set('b', 2);
  ...
  it('returns null for invalid JSON', () => {
    localStorage.setItem('pos_corrupt', '{invalid');
    expect(storage.get('corrupt')).toBeNull();
  });

  it('prefixes all keys with pos_', () => {
    storage.set('x', 1);
    expect(localStorage.getItem('pos_x')).toBe('1');
  });

  it('ignores non-pos_ keys when listing', () => {

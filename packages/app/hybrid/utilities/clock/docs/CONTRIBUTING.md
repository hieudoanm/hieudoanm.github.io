...
Thanks for contributing to **Clock**, a hybrid app that ships as a web app
...
3. **Run this app**:
   ...
   ```bash
   pnpm dev --filter=@hieudoanm.github.io/clock
   ```
...
| Task       | Command                                             |
| ---------- | --------------------------------------------------- |
| Dev server | `pnpm dev --filter=@hieudoanm.github.io/clock`      |
| Build      | `pnpm build --filter=@hieudoanm.github.io/clock`    |
| Lint       | `pnpm lint --filter=@hieudoanm.github.io/clock`     |
| Format     | `pnpm format --filter=@hieudoanm.github.io/clock`   |
| Unit tests | `pnpm test --filter=@hieudoanm.github.io/clock`     |
| E2E tests  | `pnpm test:e2e --filter=@hieudoanm.github.io/clock` |
...
4. **DRY** — centralize repeated patterns.
...
9. Favour `zod` for runtime validation at the boundary.
...
3. nothing theme: black, white, red only.
4. `prettier-plugin-tailwindcss` sorts classes.
5. Use `react-icons` Feather (`Fi`) set for icons.

### Atomic Design

- `atoms/` — smallest building blocks (ThemeToggle)
- `molecules/` — combinations of atoms (WeatherBadge, CityCard)
- `organisms/` — complex sections (Pomodoro, Watchface, WorldClock, Header)
- `templates/` — page-level layouts (ClockApp, AboutTemplate)
...
5. Use `it.each` for data-driven tests.
...
1. `pnpm lint --filter=@hieudoanm.github.io/clock`
2. `pnpm format --filter=@hieudoanm.github.io/clock`
3. `pnpm test --filter=@hieudoanm.github.io/clock`
4. `pnpm test:e2e --filter=@hieudoanm.github.io/clock`
5. `pnpm build --filter=@hieudoanm.github.io/clock`

...
Thanks for contributing to **Password**, a hybrid app that ships as a web app
...
3. **Run this app**:
   ...
   ```bash
   pnpm dev --filter=@hieudoanm.github.io/password
   ```
...
- `organisms/` — complex sections (Header, Sidebar, VaultList)
- `templates/` — page-level layouts (VaultTemplate, SettingsTemplate)
...
1. `pnpm lint --filter=@hieudoanm.github.io/password`
2. `pnpm format --filter=@hieudoanm.github.io/password`
3. `pnpm test --filter=@hieudoanm.github.io/password`
4. `pnpm test:e2e --filter=@hieudoanm.github.io/password`
5. `pnpm build --filter=@hieudoanm.github.io/password`

...
const STORAGE_KEY = 'password-theme';

const getInitialTheme = (): string => {
  if (typeof window === 'undefined') return 'password-light';
  return localStorage.getItem(STORAGE_KEY) || 'password-light';
};
...
export const Header: FC = () => {
  ...
  const toggleTheme = () =>
    setTheme((current) =>
      current === 'password-light' ? 'password-dark' : 'password-light'
    );
      ...
      <div className="mx-auto flex max-w-5xl items-center justify-between">
        ...
        <nav className="flex items-center gap-0.5">
          ...
          <button
            ...
            {theme === 'password-dark' ? <FiSun /> : <FiMoon />}
          </button>
        </nav>
      </div>
    </header>

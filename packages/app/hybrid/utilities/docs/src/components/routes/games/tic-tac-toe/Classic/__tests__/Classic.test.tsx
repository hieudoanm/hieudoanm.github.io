...
describe('Classic', () => {
  it('should render correctly', () => {
    const { container } = render(<Classic onClose={jest.fn()} />);
  ...
  it('shows draw when board is full without winner', () => {
    const { container } = render(<Classic onClose={jest.fn()} />);
    // X O O
    // O X X
    // X X O
    ...
    clickCell(container, 2); // O
    clickCell(container, 5); // X
    clickCell(container, 3); // O
    clickCell(container, 6); // X
    clickCell(container, 8); // O

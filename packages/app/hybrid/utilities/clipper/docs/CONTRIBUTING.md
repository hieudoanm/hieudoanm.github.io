...
Thanks for contributing to **Clipper**, a hybrid clipboard manager that ships as
a web app (browser), desktop app (Tauri), and mobile app (Tauri Mobile).

## Getting Started

1. **Prerequisites**: Node.js (see `.nvmrc` at the repo root), `pnpm`, and Rust
   (latest stable).
...
3. **Run this app**:
   ...
   ```bash
   pnpm dev --filter=@hieudoanm.github.io/clipper
   ```

4. **Run the Tauri desktop app** (in a separate terminal):

   ```bash
   pnpm tauri --filter=@hieudoanm.github.io/clipper dev
   ```
...
| Task        | Command                                                  |
| ----------- | -------------------------------------------------------- |
| Dev server  | `pnpm dev --filter=@hieudoanm.github.io/clipper`         |
| Build       | `pnpm build --filter=@hieudoanm.github.io/clipper`       |
| Lint        | `pnpm lint --filter=@hieudoanm.github.io/clipper`        |
| Format      | `pnpm format --filter=@hieudoanm.github.io/clipper`      |
| Unit tests  | `pnpm test --filter=@hieudoanm.github.io/clipper`        |
| E2E tests   | `pnpm test:e2e --filter=@hieudoanm.github.io/clipper`    |
| Tauri dev   | `pnpm tauri --filter=@hieudoanm.github.io/clipper dev`   |
| Tauri build | `pnpm tauri --filter=@hieudoanm.github.io/clipper build` |
...
3. Dark theme is the default (`data-theme="clipper"`).
...
- `atoms/` — smallest building blocks (Button, Badge, Input) — reserved
- `molecules/` — combinations of atoms — reserved
- `organisms/` — complex sections (ClipboardHistory)
- `templates/` — page-level layouts (ErrorTemplate, NotFoundTemplate)
...
6. Mock `react-icons/fi` in component tests (icons don't render in jsdom).
7. Maintain ≥80% coverage on statements, branches, functions, and lines —
...
1. Use `Result<T, E>` for fallible functions — never `panic!` in production.
2. Use `#[tauri::command]` for IPC handlers and register via
   `generate_handler![]`.
3. Use `State<'_, T>` for shared state — managed via `.manage()` in setup.
4. Use `serde` for serialization between Rust and TypeScript.
5. Keep Tauri commands thin — delegate to helper functions for business logic.
6. Use `chrono` for timestamps, `rusqlite` for database access.
7. Run `cargo clippy` and `cargo fmt` before pushing.

## Before You Push

1. `pnpm lint --filter=@hieudoanm.github.io/clipper`
2. `pnpm format --filter=@hieudoanm.github.io/clipper`
3. `pnpm test --filter=@hieudoanm.github.io/clipper`
4. `pnpm build --filter=@hieudoanm.github.io/clipper`

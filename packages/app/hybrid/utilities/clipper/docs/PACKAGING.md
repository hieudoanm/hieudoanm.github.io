# Clipper — Packaging Checklist

Checklist for shipping Clipper to users: macOS notarization, Windows
code-signing, Android/iOS store builds, and the Tauri updater pipeline.

App: `clipper` (bundle identifier `io.github.hieudoanm.clipper`, version 0.0.1).
...
- [ ] Endpoint to publish to:
      `https://github.com/hieudoanm/hieudoanm.github.io/releases/download/app-hybrid-utilities-clipper-latest/latest.json`
...
- [ ] Verify clipboard monitoring works in sandboxed app (requires Accessibility
...
- [ ] Signing team + provisioning profile set for the `clipper` Xcode project

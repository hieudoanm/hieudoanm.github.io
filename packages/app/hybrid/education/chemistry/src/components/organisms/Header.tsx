...
const getInitialTheme = (): string => {
  if (typeof window === 'undefined') return 'chemistry-light';
  return localStorage.getItem(STORAGE_KEY) || 'chemistry-light';
};
...
export const Header: FC = () => {
  ...
  const toggleTheme = () =>
    setTheme((current) =>
      current === 'chemistry-light' ? 'chemistry-dark' : 'chemistry-light'
    );
      ...
      <div className="mx-auto flex max-w-5xl items-center justify-between">
        ...
        <nav className="flex items-center gap-0.5">
          ...
          <button
            ...
            {theme === 'chemistry-dark' ? <FiSun /> : <FiMoon />}
          </button>
        </nav>
      </div>
    </header>

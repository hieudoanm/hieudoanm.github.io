...
describe('ToolTemplate', () => {
  it('renders a back link, the title and children', () => {
    render(
      <ToolTemplate title="Music tool">
        <p>content</p>
      </ToolTemplate>
    );
    expect(
      screen.getByRole('heading', { name: 'Music tool' })
    ).toBeInTheDocument();
...
describe('VersionTemplate', () => {
  it('renders date segments for a timestamp version', async () => {
    ...
    render(<VersionTemplate appName="Music" version="2026.08.09.10.20.30" />);
  ...
  it('falls back to raw version text for non-timestamp versions', () => {
    render(<VersionTemplate appName="Music" version="v0.0.1" />);

...
describe('ToolTemplate', () => {
  it('renders a back link, the title and children', () => {
    render(
      <ToolTemplate title="Economics tool">
        <p>content</p>
      </ToolTemplate>
    );
    expect(
      screen.getByRole('heading', { name: 'Economics tool' })
    ).toBeInTheDocument();
...
describe('ThemeToggle', () => {
  it('switches between light and dark themes', () => {
    ...
    expect(document.documentElement.dataset.theme).toBe('economics');
...
describe('VersionTemplate', () => {
  it('renders date segments for a timestamp version', async () => {
    ...
    render(
      <VersionTemplate appName="Economics" version="2026.08.09.10.20.30" />
    );
  ...
  it('falls back to raw version text for non-timestamp versions', () => {
    render(<VersionTemplate appName="Economics" version="v0.0.1" />);

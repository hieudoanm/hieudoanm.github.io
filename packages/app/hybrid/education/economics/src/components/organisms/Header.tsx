...
const STORAGE_KEY = 'economics-theme';

const getInitialTheme = (): string => {
  if (typeof window === 'undefined') return 'economics-light';
  return localStorage.getItem(STORAGE_KEY) || 'economics-light';
};
...
export const Header: FC = () => {
  ...
  const toggleTheme = () =>
    setTheme((current) =>
      current === 'economics-light' ? 'economics-dark' : 'economics-light'
    );
      ...
      <div className="mx-auto flex max-w-5xl items-center justify-between">
        ...
        <nav className="flex items-center gap-0.5">
          ...
          <button
            ...
            {theme === 'economics-dark' ? <FiSun /> : <FiMoon />}
          </button>
        </nav>
      </div>
    </header>

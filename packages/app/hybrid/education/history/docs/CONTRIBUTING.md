...
pnpm install # repo root pnpm dev --filter=@hieudoanm.github.io/history # dev
server on :3000
...
- Features are self-contained folders: pure logic in `engine.ts` (no React),
  state in `store.ts` (Zustand), UI in `index.tsx` and `components/`.
...
- Name tests as behaviour specs; pure engine functions get exhaustive edge-case
  tests.

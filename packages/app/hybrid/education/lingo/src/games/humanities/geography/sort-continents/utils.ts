...
const sortRegionOf = (entry: CountryEntry): Region | null => {
  if (entry.region === 'Americas') {
    return entry.subregion === 'South America'
      ? 'South America'
      : 'North America';
  }
  return (REGIONS as readonly string[]).includes(entry.region)
    ? (entry.region as Region)
    : null;
};

const SORT_POOL = countries.filter(
  (entry) => entry.rank > 0 && sortRegionOf(entry) !== null
);

export const regionOf = (name: string): Region | null => {
  const entry = SORT_POOL.find((candidate) => candidate.name === name);
  return entry ? sortRegionOf(entry) : null;
};
...
export const pickSortCards = (count = SORT_COUNT): SortCard[] => {
  ...
  return shuffled.slice(0, count).map((entry) => ({
    name: entry.name,
    flag: entry.flag,
    correctRegion: sortRegionOf(entry) as Region,
    placedIn: null,
  }));
};

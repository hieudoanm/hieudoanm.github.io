...
describe('useGame', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    localStorage.removeItem('music-high-score');
  ...
  it('restores a saved high score on mount', () => {
    localStorage.setItem('music-high-score', '42');
  ...
  it('ends the game and saves the high score after a wrong guess', async () => {
    jest.useFakeTimers();
    localStorage.setItem('music-high-score', '1');
    const { result } = renderHook(() => useGame(playTone));
    act(() => {
      result.current.startGame();
    });
    ...
    expect(localStorage.getItem('music-high-score')).toBe('3');
    jest.useRealTimers();
  });

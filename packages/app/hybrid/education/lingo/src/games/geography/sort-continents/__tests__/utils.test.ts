...
describe('continents-sort utils', () => {
  it('REGIONS lists the six continents', () => {
  ...
  it('emptyBuckets returns six empty buckets', () => {
    const buckets = emptyBuckets();
    expect(Object.keys(buckets)).toHaveLength(6);
    for (const region of REGIONS) expect(buckets[region]).toEqual([]);
  });

  it('regionOf resolves known countries and rejects unknown ones', () => {
    expect(regionOf('Chile')).toBe('South America');
    expect(regionOf('Canada')).toBe('North America');
    expect(regionOf('United States')).toBeNull();

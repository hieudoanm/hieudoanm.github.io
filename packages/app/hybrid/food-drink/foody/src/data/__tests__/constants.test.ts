...
describe('food data', () => {
  it('contains 64 foods', () => {
    expect(TOTAL_FOODS).toBe(64);
  ...
  it('gives every cuisine exactly 8 foods', () => {
    CUISINES.forEach((cuisine) => {
      expect(
        FOODS.filter((food) => food.category === cuisine.value)
      ).toHaveLength(8);
    });
  });

  it('covers Italy, Korea, Japan, Thailand, Vietnam, Mexico, USA and China', () => {

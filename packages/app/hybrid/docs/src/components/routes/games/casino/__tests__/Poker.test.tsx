jest.mock('../Poker/utils/poker', () => ({
  runSimulation: jest.fn().mockReturnValue({ hero: 3000, tie: 500 }),
}));

import { act, render, fireEvent, screen } from '@testing-library/react';
import { Poker } from '../Poker';
import { ITERATIONS } from '../Poker/constants';
import { runSimulation } from '../Poker/utils/poker';
...
describe('Poker', () => {
  ...
  it('should render correctly', () => {
    const { container } = render(<Poker onClose={jest.fn()} />);
    expect(container).toMatchSnapshot();
  });

  it('shows player selection message', () => {
    render(<Poker onClose={jest.fn()} />);

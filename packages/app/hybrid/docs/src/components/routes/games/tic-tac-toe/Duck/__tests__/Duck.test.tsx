...
describe('Duck', () => {
  ...
  it('alternates players after full turn', () => {
    ...
    expect(screen.getByText(/'s turn — place mark/)).toBeInTheDocument();
  });
  ...
  it('shows winner when three in a row', () => {
    const { container } = render(<Duck onClose={jest.fn()} />);
    clickCell(container, 0); // X at 0
    clickCell(container, 8); // duck to 8
    clickCell(container, 3); // O at 3
    clickCell(container, 7); // duck to 7
    clickCell(container, 1); // X at 1
    clickCell(container, 6); // duck to 6
    clickCell(container, 4); // O at 4
    clickCell(container, 5); // duck to 5
    clickCell(container, 2); // X at 2 — wins top row

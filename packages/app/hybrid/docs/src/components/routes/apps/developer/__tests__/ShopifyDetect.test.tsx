...
import { ShopifyDetect } from '../ShopifyDetect';

let mockHistoryStore: any[] = [];

jest.mock('../ShopifyDetect/utils/detect', () => ({
...
jest.mock('../ShopifyDetect/utils/storage', () => {
...
const { detectFromHTML } = jest.requireMock('../ShopifyDetect/utils/detect');
const { loadHistory, saveHistory, clearHistory } = jest.requireMock(
  '../ShopifyDetect/utils/storage'
);
...
describe('ShopifyDetect', () => {
  ...
  it('renders Check tab', () => {
    render(<ShopifyDetect onClose={onClose} />);

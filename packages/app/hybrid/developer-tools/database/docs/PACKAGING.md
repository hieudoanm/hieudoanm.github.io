# Database — Packaging Checklist

Checklist for shipping Database to users: macOS notarization, Windows
code-signing, Android/iOS store builds, and the Tauri updater pipeline.

App: `database` (bundle identifier `io.github.hieudoanm.database`, version
0.0.1).
...
- [ ] Endpoint to publish to:
      `https://github.com/hieudoanm/hieudoanm.github.io/releases/download/app-hybrid-database-latest/latest.json`
...
- [ ] Signing team + provisioning profile set for the `database` Xcode project

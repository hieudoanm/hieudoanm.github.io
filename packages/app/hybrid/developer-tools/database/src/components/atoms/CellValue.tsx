import { type FC } from 'react';
import type { SqliteCell } from '@/types/sqlite';

export const CellValue: FC<{ value: SqliteCell }> = ({ value }) => {
  if (value === null)
    return <span className="text-base-content/30 text-xs italic">NULL</span>;
  if (value instanceof Uint8Array)
    return (
      <span className="text-base-content/50 text-xs italic">
        [BLOB {value.length}B]
      </span>
    );
  if (typeof value === 'number')
    return <span className="text-primary font-mono">{value}</span>;
  const s = String(value);
  if (s === '1' || s.toLowerCase() === 'true')
    return <span className="text-primary font-mono text-xs">{s}</span>;
  if (s === '0' || s.toLowerCase() === 'false')
    return <span className="text-base-content/40 font-mono text-xs">{s}</span>;
  return (
    <span className="font-mono text-xs">
      {s.length > 80 ? s.slice(0, 80) + '…' : s}
    </span>
  );
};
CellValue.displayName = 'CellValue';

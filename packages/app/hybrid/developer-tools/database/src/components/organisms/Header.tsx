...
const STORAGE_KEY = 'database-theme';

const getInitialTheme = (): string => {
  if (typeof window === 'undefined') return 'database-light';
  return localStorage.getItem(STORAGE_KEY) || 'database-light';
};
...
export const Header: FC = () => {
  ...
  const toggleTheme = () =>
    setTheme((current) =>
      current === 'database-light' ? 'database-dark' : 'database-light'
    );
      ...
      <div className="mx-auto flex max-w-5xl items-center justify-between">
        ...
        <nav className="flex items-center gap-0.5">
          ...
          <button
            ...
            {theme === 'database-dark' ? <FiSun /> : <FiMoon />}
          </button>
        </nav>
      </div>
    </header>

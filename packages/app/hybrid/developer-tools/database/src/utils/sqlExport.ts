...
export const cellToString = (v: SqliteCell): string => {
  if (v === null) return '';
...
export const convertToCSV = (columns: string[], rows: SqliteCell[][]): string =>
...
export const convertToJSON = (
  ...
  JSON.stringify(
    rows.map((r) => {
      const obj: Record<string, SqliteCell> = {};
      columns.forEach((c, i) => {
        obj[c] =
...
export const convertToSQL = (
  ...
  const sqlVal = (v: SqliteCell): string => {

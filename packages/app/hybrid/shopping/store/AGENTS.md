...
- Store has its own copy of `downloads.json` (and its per-section source CSVs in
  `src/data/csv/`) — not cross-package
...
```
src/app/                # App Router pages (62 app detail pages + info routes)
src/components/
  organisms/            # Header
...
src/data/
  downloads.json        # App catalog (62 apps)
...
- `/app/[slug]` — app detail page (62 static pages)

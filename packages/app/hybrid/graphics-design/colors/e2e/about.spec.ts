...
test('loads about page', async ({ page }) => {
  ...
  await expect(page.locator('p.text-xs:has-text("About")')).toBeVisible();
});
...
test('displays app description', async ({ page }) => {
  await page.goto('/about');
  await expect(
    page.locator('text=Interactive tools for picking and shipping color')
  ).toBeVisible();
});
...
test('displays styling info', async ({ page }) => {
  await page.goto('/about');
  await expect(page.locator('text=Styling')).toBeVisible();
  await expect(page.locator('text=Tailwind CSS 4 + DaisyUI 5')).toBeVisible();
});

test('displays tools count', async ({ page }) => {
  await page.goto('/about');
  await expect(page.locator('text=16 color utilities')).toBeVisible();
});

test('displays desktop info', async ({ page }) => {
  await page.goto('/about');
  await expect(page.locator('text=Desktop')).toBeVisible();
  await expect(page.locator('text=Tauri 2')).toBeVisible();
});

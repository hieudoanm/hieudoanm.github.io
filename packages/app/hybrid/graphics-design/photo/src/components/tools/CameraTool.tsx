...
import { type ImageToolConfig } from '@/data/photo-tools';
...
export const CameraTool: FC<{ config: ImageToolConfig }> = ({ config }) => {
  ...
  const handleCapture = async () => {
    ...
    if (videoRatio > targetRatio) cropWidth = videoHeight * targetRatio;
    else cropHeight = videoWidth / targetRatio;
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-6 rounded-xl bg-neutral-900 p-4 text-white">
        {error ? (
                  ...
                  {(['none', 'thirds', 'symmetry'] as OverlayMode[]).map(
                    (mode) => (
                      <button
                        ...
                        className={`join-item btn btn-xs h-8 px-4 capitalize transition-all ${overlay === mode ? 'btn-primary no-animation' : 'btn-ghost text-white/40'}`}>
...
CameraTool.displayName = 'CameraTool';

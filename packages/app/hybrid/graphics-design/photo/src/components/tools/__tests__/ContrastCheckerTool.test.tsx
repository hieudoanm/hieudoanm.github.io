...
describe('ContrastCheckerTool', () => {
  it('renders four contrast levels with PASS/FAIL', () => {
    const { container } = render(
      <ContrastCheckerTool config={cfg('contrast-checker')} />
    );
    const total = screen
      .getAllByText(/PASS|FAIL/)
      .filter(
        (n) => n.textContent === 'PASS' || n.textContent === 'FAIL'
      ).length;
    expect(total).toBe(4);
  });

  it('shows FAIL for low contrast foreground', () => {
    const { container } = render(
      <ContrastCheckerTool config={cfg('contrast-checker')} />
    );
    const fgInput = container.querySelectorAll('input[type="text"]')[0];

...
describe('CollageMakerTool', () => {
  it('renders upload area initially', () => {
    const { container } = render(<CollageMakerTool config={cfg('collage')} />);
    expect(container.querySelector('input[type="file"]')).toBeTruthy();
  });

  it('create button disabled before upload', () => {
    render(<CollageMakerTool config={cfg('collage')} />);
    expect(
      screen.getByRole('button', { name: 'Create Collage' })
    ).toBeDisabled();
  });

  it('creates collage after upload and downloads', async () => {
    const { container } = render(<CollageMakerTool config={cfg('collage')} />);
    uploadFile(container);
    await flush();
    fireEvent.click(screen.getByRole('button', { name: 'Create Collage' }));
    await flush();
    await waitFor(() =>
  ...
  it('shows upload count after two files', async () => {
    const { container } = render(<CollageMakerTool config={cfg('collage')} />);
    uploadFile(container);
    await flush();
    uploadFile(container);
    await flush();
    expect(screen.getByText('2 image(s)')).toBeTruthy();
  });
});

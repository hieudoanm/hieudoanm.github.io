...
describe('ImageProfileTool', () => {
  it('renders upload area initially', () => {
    const { container } = render(
      <ImageProfileTool config={cfg('image-profile')} />
    );
    expect(container.querySelector('input[type="file"]')).toBeTruthy();
  });

  it('generates profile after upload and downloads', async () => {
    const { container } = render(
      <ImageProfileTool config={cfg('image-profile')} />
    );
    ...
    await flush();
    fireEvent.click(
      screen.getByRole('button', { name: 'Generate LinkedIn Photo' })
    );
    await flush();
    await waitFor(() =>
      expect(mockDownloadBlob).toHaveBeenCalledWith(
        expect.any(Blob),
        'linkedin_profile_photo.png'
      )
    );
  });

  it('switches platform and changes offset', async () => {
    const { container } = render(
      <ImageProfileTool config={cfg('image-profile')} />
    );
    uploadFile(container);
    ...
    fireEvent.click(
      screen.getByRole('button', { name: 'Generate GitHub Photo' })
    );
    await flush();

...
const mockMutate = jest.fn().mockResolvedValue({ text: 'Xin chào' });
jest.mock('@/utils/trpc', () => ({
  trpcClient: {
    openrouter: { generate: { mutate: (...a: unknown[]) => mockMutate(...a) } },
  },
}));
...
describe('ImageTranslateTool', () => {
  it('renders without throwing', () => {
    const { container } = render(
      <ImageTranslateTool config={cfg('image-translate')} />
    );
    expect(container.firstChild).toBeTruthy();
  });

  it('translates pasted text and shows result block', async () => {
    render(<ImageTranslateTool config={cfg('image-translate')} />);
    fireEvent.change(
    ...
    await waitFor(() => expect(screen.getByText('Translation:')).toBeTruthy());
  });
});

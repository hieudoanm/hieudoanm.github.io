...
describe('AiUnblurTool', () => {
  it('renders upload area initially', () => {
    const { container } = render(<AiUnblurTool config={cfg('ai-unblur')} />);
    expect(container.querySelector('input[type="file"]')).toBeTruthy();
  });

  it('deblurs after upload and downloads', async () => {
    const { container } = render(<AiUnblurTool config={cfg('ai-unblur')} />);
    uploadFile(container);
    await flush();
    fireEvent.click(screen.getByRole('button', { name: 'Apply Deblur' }));
    await waitFor(() =>

...
const hex2oklch = (hex: string) => {
  ...
  return {
    ...
    c:
      Math.round(
        Math.sqrt(
          (1.9779984951 * m_ - 2.428592205 * l_ - 0.4505927117 * s_) ** 2 +
            (-0.0259040371 * l_ - 0.7827717662 * m_ - 0.808675766 * s_) ** 2
        ) * 100
      ) / 100,
    h: Math.round(
      (Math.atan2(
        -0.0259040371 * l_ - 0.7827717662 * m_ + 0.808675766 * s_,
        1.9779984951 * m_ - 2.428592205 * l_ - 0.4505927117 * s_
      ) *
        180) /
        Math.PI
    ),
...
export const ColorsTool: FC<{ config: ImageToolConfig }> = ({ config }) => {

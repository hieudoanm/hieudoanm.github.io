...
describe('BarcodeTool', () => {
  it('renders without throwing', () => {
    const { container } = render(<BarcodeTool config={cfg('barcode')} />);
    expect(container.firstChild).toBeTruthy();
  });

  it('gen button disabled with empty input', () => {
    render(<BarcodeTool config={cfg('barcode')} />);
    expect(screen.getByRole('button', { name: 'Gen' })).toBeDisabled();
  });

  it('changes format select', () => {
    render(<BarcodeTool config={cfg('barcode')} />);
    const select = screen.getByRole('combobox');
    fireEvent.change(select, { target: { value: 'EAN13' } });
  });

  it('shows download button after generating', async () => {
    const { container } = render(<BarcodeTool config={cfg('barcode')} />);
    fireEvent.change(screen.getByPlaceholderText('Enter text for barcode'), {

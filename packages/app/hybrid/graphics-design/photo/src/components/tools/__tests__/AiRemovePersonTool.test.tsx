...
describe('AiRemovePersonTool', () => {
  it('renders upload area initially', () => {
    const { container } = render(
      <AiRemovePersonTool config={cfg('ai-remove-person')} />
    );
    expect(container.querySelector('input[type="file"]')).toBeTruthy();
  });

  it('remove button disabled until click position set', async () => {
    const { container } = render(
      <AiRemovePersonTool config={cfg('ai-remove-person')} />
    );
  ...
  it('removes person after canvas click and downloads', async () => {
    const { container } = render(
      <AiRemovePersonTool config={cfg('ai-remove-person')} />
    );
    uploadFile(container);
    ...
    const display =
      canvases.find((c) => !(c as HTMLElement).classList.contains('hidden')) ??
      canvases[0];
    fireEvent.click(display, { clientX: 50, clientY: 50 });

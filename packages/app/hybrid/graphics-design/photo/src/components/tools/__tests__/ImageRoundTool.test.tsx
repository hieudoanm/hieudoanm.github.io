...
describe('ImageRoundTool', () => {
  ...
  it('makes round after upload and downloads', async () => {
    const { container } = render(
      <ImageRoundTool config={cfg('image-round')} />
    );
    uploadFile(container);
    await flush();
    fireEvent.click(screen.getByRole('button', { name: 'Make Round' }));
    await waitFor(() =>
  ...
  it('changes radius slider and processes', async () => {
    const { container } = render(
      <ImageRoundTool config={cfg('image-round')} />
    );
    uploadFile(container);

...
describe('AiColorizeTool', () => {
  it('renders upload area and disabled apply', () => {
    render(<AiColorizeTool config={cfg('ai-colorize')} />);
    expect(
      screen.getByRole('button', { name: 'Apply Colorize' })
    ).toBeDisabled();
  });

  it('applies solid colorize after upload and downloads', async () => {
    const { container } = render(
      <AiColorizeTool config={cfg('ai-colorize')} />
    );
    uploadFile(container);
    await flush();
    fireEvent.click(screen.getByRole('button', { name: 'Apply Colorize' }));
    await waitFor(() =>
  ...
  it('applies gradient colorize and changes sliders', async () => {
    const { container } = render(
      <AiColorizeTool config={cfg('ai-colorize')} />
    );
    fireEvent.click(
      screen.getAllByRole('button').find((b) => b.textContent === 'Gradient')!
    );
    uploadFile(container);
    await flush();
  ...
  it('toBlob null does not download', async () => {

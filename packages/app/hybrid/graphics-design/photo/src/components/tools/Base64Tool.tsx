...
const isImageMime = (mime: string) =>
  /^image\/(png|jpe?g|gif|webp|svg\+xml|bmp|avif)$/.test(mime);
const sniffMime = (raw: string): string | null => {
...
const dataUrlFromMime = (b64: string, mime: string) =>
  `data:${mime};base64,${b64}`;

type Tab = 'decode' | 'encode';

export const Base64Tool: FC<{ config: ImageToolConfig }> = ({ config }) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {tab === 'encode' && (
        <div className="mt-4">
          <ImageFileUpload accept="*/*" onFile={readFileAsBase64} />
        </div>
      )}
      <textarea

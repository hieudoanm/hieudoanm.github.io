...
describe('AiGenerateTool', () => {
  ...
  it('changes style and size then generates', async () => {
    render(<AiGenerateTool config={cfg('ai-generate')} />);
    fireEvent.change(
      screen.getByPlaceholderText('Describe the image you want to generate...'),
      {
    ...
    fireEvent.click(
      screen.getAllByRole('button').find((b) => b.textContent === 'Watercolor')!
    );
    fireEvent.click(
      screen.getAllByRole('button').find((b) => b.textContent === 'Watercolor')!
    );
    fireEvent.click(
      screen.getAllByRole('button').find((b) => b.textContent === 'Cyberpunk')!
    );
    fireEvent.click(
      screen.getAllByRole('button').find((b) => b.textContent === 'Anime')!
    );
    fireEvent.click(
      screen.getAllByRole('button').find((b) => b.textContent === '3D Render')!
    );
    fireEvent.click(
      screen.getAllByRole('button').find((b) => b.textContent === 'Sketch')!
    );
    fireEvent.click(
      screen.getAllByRole('button').find((b) => b.textContent === '1024×1024')!
    );
    fireEvent.click(screen.getByRole('button', { name: 'Generate' }));
    await waitFor(
      () =>
  ...
  it('early returns when generating with empty prompt', () => {

...
describe('PixelTool', () => {
  it('renders upload area initially', () => {
    const { container } = render(<PixelTool config={cfg('pixel')} />);
    expect(container.querySelector('input[type="file"]')).toBeTruthy();
  });

  it('shows pixel controls after upload', async () => {
    const { container } = render(<PixelTool config={cfg('pixel')} />);
    uploadFile(container);
    await flush();
    expect(
      screen.getAllByRole('button').some((b) => b.textContent === '32×32')
    ).toBe(true);
    expect(
      screen.getAllByRole('button').some((b) => b.textContent === 'Square')
    ).toBe(true);
    expect(
      screen.getAllByRole('button').some((b) => b.textContent === 'Pixel')
    ).toBe(true);
  });

  it('switches grid size and mode after upload', async () => {
    const { container } = render(<PixelTool config={cfg('pixel')} />);
    uploadFile(container);
    await flush();
    const getBtn = (t: string) =>
      screen.getAllByRole('button').find((b) => b.textContent === t)!;
    fireEvent.click(getBtn('64×64'));
    fireEvent.click(getBtn('Scale'));
    expect(
      screen.getAllByRole('button').some((b) => b.textContent === 'Original')
    ).toBe(true);
  });
});

...
describe('AiUpscaleTool', () => {
  it('renders without throwing', () => {
    const { container } = render(
      <AiUpscaleTool config={makeCfg('AiUpscaleTool')} />
    );
  ...
  it('processes after upload and downloads', async () => {
    ...
    if (btn && !btn.disabled) expect(mockDownloadBlob).toHaveBeenCalled();
  });

  it('does not throw before upload', () => {
    render(<AiUpscaleTool config={makeCfg('AiUpscaleTool')} />);
  });
});

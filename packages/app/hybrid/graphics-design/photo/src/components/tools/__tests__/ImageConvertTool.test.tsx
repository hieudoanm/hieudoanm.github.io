...
describe('ImageConvertTool', () => {
  it('renders upload area initially', () => {
    const { container } = render(
      <ImageConvertTool config={cfg('image-convert')} />
    );
    expect(container.querySelector('input[type="file"]')).toBeTruthy();
  });

  it('converts on upload and downloads', async () => {
    const { container } = render(
      <ImageConvertTool config={cfg('image-convert')} />
    );
  ...
  it('renders the conversion description', () => {
    render(<ImageConvertTool config={cfg('image-convert')} />);
    expect(screen.getByText(/Convert to/)).toBeTruthy();
  });

  it('converts via SVG when config.isSvg is set', async () => {
    const { container } = render(
      <ImageConvertTool
        config={{ ...cfg('image-convert'), isSvg: true, outputExt: 'webp' }}
      />
    );
    uploadFile(container);
    await flush();
  ...
  it('skips download when toBlob yields null', async () => {

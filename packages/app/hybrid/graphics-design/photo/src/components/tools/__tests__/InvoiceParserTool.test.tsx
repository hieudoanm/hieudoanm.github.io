...
describe('InvoiceParserTool', () => {
  it('renders upload area initially', () => {
    const { container } = render(
      <InvoiceParserTool config={cfg('invoice-parser')} />
    );
    expect(screen.getByText('Upload Invoice')).toBeTruthy();
    expect(container.querySelector('input[type="file"]')).toBeTruthy();
  });

  it('run OCR button disabled before upload', () => {
    render(<InvoiceParserTool config={cfg('invoice-parser')} />);
    expect(screen.getByRole('button', { name: 'Run OCR + AI' })).toBeDisabled();
  });

  it('runs OCR after upload and extracts vendor', async () => {
    const { container } = render(
      <InvoiceParserTool config={cfg('invoice-parser')} />
    );
    uploadFile(container);
    await flush();
    fireEvent.click(screen.getByRole('button', { name: 'Run OCR + AI' }));
    await waitFor(() => expect(screen.getByText('Acme Corp')).toBeTruthy());
  });

  it('extracts total from OCR text', async () => {
    const { container } = render(
      <InvoiceParserTool config={cfg('invoice-parser')} />
    );
    uploadFile(container);

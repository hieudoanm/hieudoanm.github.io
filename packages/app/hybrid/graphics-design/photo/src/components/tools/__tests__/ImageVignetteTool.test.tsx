...
describe('ImageVignetteTool', () => {
  ...
  it('button disabled before upload', () => {
    render(<ImageVignetteTool config={cfg('image-vignette')} />);
    expect(
      screen.getByRole('button', { name: /Apply Vignette/ })
    ).toBeDisabled();
  });

  it('applies vignette and downloads', async () => {
    const { container } = render(
      <ImageVignetteTool config={cfg('image-vignette')} />
    );
  ...
  it('intensity slider changes and processes', async () => {
    const { container } = render(
      <ImageVignetteTool config={cfg('image-vignette')} />
    );
    uploadFile(container);

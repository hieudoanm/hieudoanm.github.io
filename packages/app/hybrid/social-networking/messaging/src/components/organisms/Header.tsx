...
const getInitialTheme = (): string => {
  if (typeof window === 'undefined') return 'messaging-light';
  return localStorage.getItem(STORAGE_KEY) || 'messaging-light';
};
...
export const Header: FC = () => {
  ...
  const toggleTheme = () =>
    setTheme((current) =>
      current === 'messaging-light' ? 'messaging-dark' : 'messaging-light'
    );
      ...
      <div className="mx-auto flex max-w-5xl items-center justify-between">
        ...
        <nav className="flex items-center gap-0.5">
          ...
          <button
            ...
            {theme === 'messaging-dark' ? <FiSun /> : <FiMoon />}
          </button>
        </nav>
      </div>
    </header>

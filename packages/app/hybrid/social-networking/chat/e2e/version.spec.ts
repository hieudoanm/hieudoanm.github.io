...
test.describe('Version Page', () => {
  ...
  test('loads version page', async ({ page }) => {
    ...
    await expect(page.locator('h1:has-text("Chat Version")')).toBeVisible();
  });

  test('displays version in YYYY.MM.DD format', async ({ page }) => {
    await page.goto('/version');
    await expect(
  ...
  test('clicking copy version button changes text to Copied', async ({
    page,
  }) => {
    await page.goto('/version');
  ...
  test('version reverts back from Copied after delay', async ({ page }) => {
    await page.goto('/version');
    await page.locator('button:has-text("Copy version")').click();
    await expect(page.locator('button:has-text("Copied")')).toBeVisible();
    await expect(page.locator('button:has-text("Copy version")')).toBeVisible({

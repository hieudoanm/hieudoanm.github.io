...
const STORAGE_KEY = 'football-theme';

const getInitialTheme = (): string => {
  if (typeof window === 'undefined') return 'football-light';
  return localStorage.getItem(STORAGE_KEY) || 'football-light';
};
...
export const Header: FC = () => {
  ...
  const toggleTheme = () =>
    setTheme((current) =>
      current === 'football-light' ? 'football-dark' : 'football-light'
    );
      ...
      <div className="mx-auto flex max-w-5xl items-center justify-between">
        ...
        <nav className="flex items-center gap-0.5">
          ...
          <button
            ...
            {theme === 'football-dark' ? <FiSun /> : <FiMoon />}
          </button>
        </nav>
      </div>
    </header>

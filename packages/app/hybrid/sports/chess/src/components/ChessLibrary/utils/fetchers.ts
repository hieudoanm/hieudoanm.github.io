...
export const fetchLichessPgn = async (
  username: string,
  max = 20
): Promise<string> => {
...
export const fetchChessComPgn = async (
  username: string,
  max = 20
): Promise<string> => {
  ...
  if (!archivesResponse.ok)
    throw new Error(`Chess.com returned ${archivesResponse.status}`);
  ...
  for (const month of months) {
    ...
    const payload = (await monthResponse.json()) as {
      games?: { pgn?: string }[];
    };

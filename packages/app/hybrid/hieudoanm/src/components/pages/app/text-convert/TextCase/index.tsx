'use client';

import { FC, useState, useMemo } from 'react';
import { FullScreen } from '@hieudoanm.github.io/components/atoms/FullScreen';

export const TextCase: FC<{ onClose: () => void }> = ({ onClose }) => {
  const [inputText, setInputText] = useState('');

  const caseConverted = useMemo(() => {
    if (!inputText) return null;
    return {
      upper: inputText.toUpperCase(),
      lower: inputText.toLowerCase(),
      title: inputText.replace(
        /\w\S*/g,
        (w) => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()
      ),
      camel: inputText.replace(/[^a-zA-Z0-9]+(.)/g, (_, c) => c.toUpperCase()),
      snake: inputText.replace(/\s+/g, '_').toLowerCase(),
      kebab: inputText.replace(/\s+/g, '-').toLowerCase(),
    } as const;
  }, [inputText]);

  return (
    <FullScreen centered onClose={onClose} title="Case Converter">
      <div className="rounded-box border-base-300 bg-base-200 border p-4">
        <div className="flex flex-col gap-4">
          <textarea
            className="textarea textarea-bordered min-h-[100px]"
            placeholder="Type text to convert..."
            value={inputText}
            onChange={(e) => setInputText(e.target.value)}
          />
          {inputText && (
            <div className="bg-base-200 space-y-2 rounded p-4 text-sm">
              <div>
                <span className="font-normal">UPPER:</span>{' '}
                {caseConverted!.upper}
              </div>
              <div>
                <span className="font-normal">lower:</span>{' '}
                {caseConverted!.lower}
              </div>
              <div>
                <span className="font-normal">Title:</span>{' '}
                {caseConverted!.title}
              </div>
              <div>
                <span className="font-normal">camelCase:</span>{' '}
                {caseConverted!.camel}
              </div>
              <div>
                <span className="font-normal">snake_case:</span>{' '}
                {caseConverted!.snake}
              </div>
              <div>
                <span className="font-normal">kebab-case:</span>{' '}
                {caseConverted!.kebab}
              </div>
            </div>
          )}
        </div>
      </div>
    </FullScreen>
  );
};
TextCase.displayName = 'TextCase';

...
export const DownloadsTemplate: FC<{
  ...
  <div className="mx-auto flex w-full max-w-2xl flex-col items-center gap-6 px-6 py-16 text-center">
    <p className="text-base-content/50 text-xs uppercase tracking-[0.2em]">Downloads</p>
        ...
        {items.map(({ platform, requirements, label, href }) => (
          <div key={`${platform}-${label}`} className="flex items-center justify-between gap-4">
            <div className="text-left">
              <span className="text-base-content block text-sm font-bold">{platform}</span>
              <span className="text-base-content/50 block text-xs">{requirements}</span>
            </div>
            <Link
              href={href}

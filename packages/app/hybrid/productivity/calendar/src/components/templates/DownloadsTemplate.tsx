...
export const DownloadsTemplate: FC<{
  ...
  <div className="flex h-full flex-col items-center justify-center px-6 py-24">
    <main className="mx-auto flex w-full max-w-2xl flex-1 flex-col items-center gap-6 p-6 text-center">
      <p className="text-base-content/50 text-xs tracking-[0.2em] uppercase">Downloads</p>

      <h1>Installers</h1>

      <p className="text-base-content/50 max-w-sm text-center text-sm">
        Pick the package for your platform. The mobile web app is available from any browser.
      </p>
      ...
      <div className="border-base-content/10 bg-base-200 mb-8 w-full max-w-lg rounded-2xl border p-6">
        <div className="flex flex-col gap-4">
          {items.map(({ platform, requirements, label, href }) => (
            <div key={`${platform}-${label}`} className="flex items-center justify-between gap-4">
              <div className="flex flex-col items-start text-left">
                <span className="text-base-content text-sm font-bold">{platform}</span>
                <span className="text-base-content/50 text-xs">{requirements}</span>
              </div>
              <Link href={href} className="btn btn-primary btn-sm" aria-label={`Download ${label}`}>
                {label}
              </Link>
            </div>
          ))}

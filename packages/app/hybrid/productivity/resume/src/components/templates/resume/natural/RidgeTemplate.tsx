...
export const RidgeTemplate: FC<TemplateProps> = ({ data }) => {
  const {
    personal,
    summary,
    experience,
  ...
  return (
    <div
      ...
      <header
        style={{
          background:
            'repeating-linear-gradient(135deg, #292524 0 18px, #44403c 18px 26px)',
          color: '#ffffff',
          padding: '32px 36px',
        }}>
        <h1
        ...
        <div style={{ fontSize: 12, color: '#d6d3d1', marginBottom: 10 }}>
          {personal.jobTitle}
        </div>
        <ContactList
          items={collectContact(data)}
          style={{ fontSize: 9.5, color: '#e7e5e4' }}
        />
      </header>
...
RidgeTemplate.displayName = 'RidgeTemplate';

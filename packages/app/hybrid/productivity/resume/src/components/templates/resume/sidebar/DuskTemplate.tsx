...
const ink = '#1e1b4b';
const DEFAULT_ACCENT = '#a21caf';
const muted = '#6b7280';

const sectionStyle = { marginBottom: 16 } as const;

export const DuskTemplate: FC<TemplateProps> = ({ data, options }) => {
  ...
  return (
    <div
      ...
      <header
        style={{
          background: `linear-gradient(120deg, #4c1d95, ${accent})`,
          color: '#ffffff',
          padding: '30px 40px',
        }}>
        <h1
        ...
        <div style={{ fontSize: 12, color: '#e9d5ff', marginBottom: 10 }}>
          {personal.jobTitle}
        </div>
        <ContactList
          items={collectContact(data)}
          style={{ fontSize: 9.5, color: '#ede9fe' }}
        />
      </header>
...
DuskTemplate.displayName = 'DuskTemplate';

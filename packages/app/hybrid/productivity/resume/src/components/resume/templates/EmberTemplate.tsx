...
export const EmberTemplate: FC<TemplateProps> = ({ data, options }) => {
  const ember = options?.accentColor || DEFAULT_EMBER;
  const titleStyle = {
    color: ember,
    fontSize: 11,
    letterSpacing: 1.8,
  ...
  return (
    <div
      ...
      <header
        style={{
          background: 'linear-gradient(100deg, #7c2d12, #c2410c)',
          color: '#ffffff',
          padding: '30px 40px',
        }}>
        <h1
        ...
        <div style={{ fontSize: 12, color: '#fed7aa', marginBottom: 10 }}>
          {personal.jobTitle}
        </div>
        <ContactList
          items={collectContact(data)}
          style={{ fontSize: 9.5, color: '#ffedd5' }}
        />
      </header>
              ...
              <div key={item.id} style={{ marginBottom: 12 }}>
                <HeaderRow
                  ...
                  right={[item.startDate, item.endDate]
                    .filter(Boolean)
                    .join(' – ')}
                  primaryStyle={{ fontSize: 12, color: ember }}
          ...
          <Section title="Skills" style={sectionStyle} titleStyle={titleStyle}>
            {skills.map((group) => (
              <div key={group.id} style={{ fontSize: 10.5, marginBottom: 4 }}>
                <strong style={{ color: ember }}>{group.category}: </strong>
                {group.items}
              </div>
            ))}
          </Section>
...
EmberTemplate.displayName = 'EmberTemplate';

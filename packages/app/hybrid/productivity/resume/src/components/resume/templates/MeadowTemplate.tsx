...
export const MeadowTemplate: FC<TemplateProps> = ({ data, options }) => {
  const green = options?.accentColor || DEFAULT_ACCENT;
  const titleStyle = {
    color: green,
    fontSize: 11,
    letterSpacing: 1.8,
  ...
  return (
    <div
      ...
      <header
        ...
        <div style={{ fontSize: 12, color: '#bbf7d0', marginBottom: 10 }}>
          {personal.jobTitle}
        </div>
        <ContactList
          items={collectContact(data)}
          style={{ fontSize: 9.5, color: '#dcfce7' }}
        />
      </header>
              ...
              <div key={item.id} style={{ marginBottom: 12 }}>
                <HeaderRow
                  ...
                  right={[item.startDate, item.endDate]
                    .filter(Boolean)
                    .join(' – ')}
                  primaryStyle={{ fontSize: 12, color: green }}
          ...
          <Section title="Skills" style={sectionStyle} titleStyle={titleStyle}>
            {skills.map((group) => (
              <div key={group.id} style={{ fontSize: 10.5, marginBottom: 4 }}>
                <strong style={{ color: green }}>{group.category}: </strong>
                {group.items}
              </div>
            ))}
          </Section>
...
MeadowTemplate.displayName = 'MeadowTemplate';

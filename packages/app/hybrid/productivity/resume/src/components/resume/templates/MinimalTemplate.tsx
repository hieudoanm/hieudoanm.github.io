...
const sectionStyle = { marginBottom: 24 } as const;
const titleStyle = { color: accent, fontSize: 11 } as const;

export const MinimalTemplate: FC<TemplateProps> = ({ data }) => {
  const {
    personal,
    summary,
    experience,
        ...
        <Section
          ...
          {certifications.length > 0 && (
            <div style={{ fontSize: 10.5, marginBottom: 8 }}>
              <strong style={{ color: accent }}>Certifications: </strong>
              {certifications.map((item) => item.name).join(', ')}
            </div>
          )}
          {languages.length > 0 && (
            <div style={{ fontSize: 10.5 }}>
              <strong style={{ color: accent }}>Languages: </strong>
              {languages
                .map((item) =>
                  item.proficiency

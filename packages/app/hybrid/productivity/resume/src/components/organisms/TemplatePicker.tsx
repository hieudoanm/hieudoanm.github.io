...
import { RESUME_TEMPLATES, TEMPLATE_GROUPS } from '../templates/resume';
import { filterTemplates, TEMPLATE_CATEGORIES } from './templateFilters';
import type { TemplateCategory } from './templateFilters';
import { TemplateThumbnail } from '../molecules/TemplateThumbnail';
import { WebTag } from '../atoms/WebTag';
...
export const TemplatePicker: FC<TemplatePickerProps> = ({
  ...
  const { matches, groups } = useMemo(() => {
    const filtered = filterTemplates(RESUME_TEMPLATES, query, category);
    return {
      matches: filtered,
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div aria-label="Template results" className="grid grid-cols-1 gap-2">
        {groups.map(({ group, templates }, index) => (
          <div key={group.id} className="contents">
            <div
              role="presentation"
              className="col-span-full mt-3 flex items-center gap-2 first:mt-0">
              <span className="bg-base-300 h-px flex-1" />
              <h3 className="text-base-content/50 text-[10px] font-bold tracking-widest uppercase">
                {group.label}
              </h3>
              <span className="bg-base-300 h-px flex-1" />
            </div>
...
export const TaggedTemplatePicker = WebTag('template-picker', TemplatePicker);

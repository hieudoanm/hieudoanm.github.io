...
const getInitialTheme = (): string => {
  if (typeof window === 'undefined') return 'resume-light';
  return localStorage.getItem(STORAGE_KEY) || 'resume-light';
};
...
export const Header: FC = () => {
  ...
  const toggleTheme = () =>
    setTheme((current) =>
      current === 'resume-light' ? 'resume-dark' : 'resume-light'
    );
      ...
      <div className="mx-auto flex max-w-5xl items-center justify-between">
        ...
        <nav className="flex items-center gap-0.5">
          ...
          <button
            ...
            {theme === 'resume-dark' ? <FiSun /> : <FiMoon />}
          </button>
        </nav>
      </div>
    </header>

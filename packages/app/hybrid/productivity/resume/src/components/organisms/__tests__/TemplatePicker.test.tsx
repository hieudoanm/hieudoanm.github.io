...
describe('TemplatePicker', () => {
  ...
  it('renders a divider with a title for every group', () => {
    renderPicker();
  ...
  it('hides empty groups when the search filters them out', () => {
    renderPicker();
    const input = screen.getByLabelText('Search templates');
    fireEvent.change(input, { target: { value: 'serif' } });
    expect(
      within(results()).getByRole('heading', { name: 'Classic & Formal' })
    ).toBeInTheDocument();
    expect(
      within(results()).queryByRole('heading', { name: 'Nature & Earth' })

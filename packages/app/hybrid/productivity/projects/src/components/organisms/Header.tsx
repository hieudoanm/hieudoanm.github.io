...
const getInitialTheme = (): string => {
  if (typeof window === 'undefined') return 'projects-light';
  return localStorage.getItem(STORAGE_KEY) || 'projects-light';
};
...
export const Header: FC = () => {
  ...
  const toggleTheme = () =>
    setTheme((current) =>
      current === 'projects-light' ? 'projects-dark' : 'projects-light'
    );
      ...
      <div className="mx-auto flex max-w-5xl items-center justify-between">
        ...
        <nav className="flex items-center gap-0.5">
          ...
          <button
            ...
            {theme === 'projects-dark' ? <FiSun /> : <FiMoon />}
          </button>
        </nav>
      </div>
    </header>

...
const STORAGE_KEY = 'markdown-theme';

const getInitialTheme = (): string => {
  if (typeof window === 'undefined') return 'markdown-light';
  return localStorage.getItem(STORAGE_KEY) || 'markdown-light';
};
...
export const Header: FC = () => {
  ...
  const toggleTheme = () =>
    setTheme((current) =>
      current === 'markdown-light' ? 'markdown-dark' : 'markdown-light'
    );
      ...
      <div className="mx-auto flex max-w-5xl items-center justify-between">
        ...
        <nav className="flex items-center gap-0.5">
          ...
          <button
            ...
            {theme === 'markdown-dark' ? <FiSun /> : <FiMoon />}
          </button>
        </nav>
      </div>
    </header>

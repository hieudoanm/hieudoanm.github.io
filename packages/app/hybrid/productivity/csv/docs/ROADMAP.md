...
- [x] Multiple sheets per file (`.xlsx`-style tabs) — add / remove / rename /
      switch
...
- [x] Formula support (`=SUM(A1:B2)`, `=AVG`, `=COUNT`, ...) with ranges, cell
      references, arithmetic, `MIN`/`MAX`, cycle detection
...
- [ ] Print-friendly grid view — basic `window.print()` + print CSS exists;
      dedicated layout/pagination pending
...
- [ ] Tauri desktop app build and signing — scaffold exists (`src-tauri/`),
      packaging/signing/updater not yet configured

...
Thanks for contributing to **CSV**, a hybrid app that ships as a web app
...
3. **Run this app**:
   ...
   ```bash
   pnpm dev --filter=@hieudoanm.github.io/csv
   ```
...
| Task       | Command                                           |
| ---------- | ------------------------------------------------- |
| Dev server | `pnpm dev --filter=@hieudoanm.github.io/csv`      |
| Build      | `pnpm build --filter=@hieudoanm.github.io/csv`    |
| Lint       | `pnpm lint --filter=@hieudoanm.github.io/csv`     |
| Format     | `pnpm format --filter=@hieudoanm.github.io/csv`   |
| Unit tests | `pnpm test --filter=@hieudoanm.github.io/csv`     |
| E2E tests  | `pnpm test:e2e --filter=@hieudoanm.github.io/csv` |
...
3. **Self-documenting identifiers** — `parseCsv(text)` needs no comment.
...
7. **Test names as documentation** — `test("parses quoted fields with commas")`.
...
3. Dark theme is the default (`data-theme="spreadsheet"`).
...
1. `pnpm lint --filter=@hieudoanm.github.io/csv`
2. `pnpm format --filter=@hieudoanm.github.io/csv`
3. `pnpm test --filter=@hieudoanm.github.io/csv`
4. `pnpm test:e2e --filter=@hieudoanm.github.io/csv`
5. `pnpm build --filter=@hieudoanm.github.io/csv`

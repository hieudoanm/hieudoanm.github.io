...
Thanks for contributing to **ClaudeX**, a cross-browser extension that tracks
your Claude.ai API rate-limit usage with an inline indicator and a toolbar
badge, on Chromium and Gecko browsers via both Manifest V2 and Manifest V3
builds.
...
3. **Build this extension**:
   ...
   ```bash
   pnpm build --filter=@hieudoanm.github.io/claudex
   ```
...
| Task    | Command                                                   |
| ------- | --------------------------------------------------------- |
| Build   | `pnpm build --filter=@hieudoanm.github.io/claudex`        |
| Lint    | `pnpm lint --filter=@hieudoanm.github.io/claudex`         |
| Format  | `pnpm format --filter=@hieudoanm.github.io/claudex`       |
| Web-ext | `pnpm web-ext lint --source-dir dist/v3` (also `dist/v2`) |
| Clean   | `pnpm clean --filter=@hieudoanm.github.io/claudex`        |
...
3. **Self-documenting identifiers** — `parseRawEntry()` needs no comment;
   `processData(x)` does.
...
5. **Small, focused files** — functions ≤ 30 lines, files ≤ 200 lines. The
   content and background entry points each keep one responsibility.
...
2. The fetch patch must only intercept `/rate_limits` and `/usage`; all other
   requests pass through untouched.
3. Content scripts must be idempotent — running twice must not duplicate the
   indicator widget.
4. Badge state lives in `chrome.storage.local`; the content cache lives in
...
6. Prefix debug logs with `[ClaudeX]` and keep them minimal.
...
2. **Manual matrix** — smoke-test after any change:
   ...
   - Play a couple of messages → percentages update without reload
3. **No false positives** — the fetch patch must not alter unrelated requests.

## Before You Push

1. `pnpm lint --filter=@hieudoanm.github.io/claudex`
2. `pnpm format --filter=@hieudoanm.github.io/claudex`
3. `pnpm build --filter=@hieudoanm.github.io/claudex`

...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  try {
    ...
    console.debug('[ClaudeX] parse error', e);
  }
...
const handleApiPayload = (url: string, body: unknown): void => {
...
const parseRateLimitsEndpoint = (body: unknown): boolean => {
  ...
  void mergeAndStore(patch);
  return true;
};

const parseUsageEndpoint = (body: unknown): boolean => {
...
const updateBadge = async (): Promise<void> => {
  ...
  badgeAction.setBadgeText({ text });
  badgeAction.setBadgeBackgroundColor({ color });
};

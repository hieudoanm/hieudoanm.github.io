...
const saveData = (data: LimitData): void => {
  try {
...
const tryParseObject = (body: unknown): boolean => {
  ...
  const toPeriod = (o: Record<string, unknown>): PeriodData => {
    return {
...
const pctStr = (d: PeriodData): string => {
  if (d.limit === 0) return '—';
...
const buildIndicator = (data: LimitData): HTMLElement => {
...
const renderIndicator = (data: LimitData): void => {
  injectStyles();

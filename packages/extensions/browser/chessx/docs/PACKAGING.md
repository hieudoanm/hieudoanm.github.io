# ChessX — Packaging Checklist

Checklist for shipping ChessX to users as a loadable extension: MV2/MV3
artifact builds, store submission (Chrome Web Store, Edge Add-ons, Mozilla
AMO), and the `web-ext` lint gate.

App: `chessx`, version 0.0.1. Manifest IDs:
`chessx-extension@hieudoanm.github.io` (Gecko).

## Common

- [ ] Bump `version` in `package.json` and both `public/manifest/*/manifest.json`
      in sync
- [ ] Run `pnpm build --filter=@hieudoanm.github.io/chessx` and confirm
...
- [ ] Release assets uploaded under the tag
      `extensions-browser-chessx-latest` with the naming scheme
      `chessx-v{2|3}.{crx,xpi,zip}`
...
- [ ] chess.com scope (`*://*.chess.com/*` `host_permissions`) documented in
...
- [ ] `browser_specific_settings.gecko.id` matches
      `chessx-extension@hieudoanm.github.io`
...
- [ ] Answer review questions: no remote code, no data collection, permissions
      limited to `activeTab` + `*://*.chess.com/*`

## Release Automation

- [ ] Tag a release `extensions-browser-chessx-<version>` on GitHub

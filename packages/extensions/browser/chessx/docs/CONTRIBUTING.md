...
Thanks for contributing to **ChessX**, a cross-browser extension that hides
ratings and usernames on chess.com so you can focus on the game, on Chromium
and Gecko browsers via both Manifest V2 and Manifest V3 builds.
...
3. **Build this extension**:
   ...
   ```bash
   pnpm build --filter=@hieudoanm.github.io/chessx
   ```
...
| Task    | Command                                                   |
| ------- | --------------------------------------------------------- |
| Build   | `pnpm build --filter=@hieudoanm.github.io/chessx`         |
| Lint    | `pnpm lint --filter=@hieudoanm.github.io/chessx`          |
| Format  | `pnpm format --filter=@hieudoanm.github.io/chessx`        |
| Web-ext | `pnpm web-ext lint --source-dir dist/v3` (also `dist/v2`) |
| Clean   | `pnpm clean --filter=@hieudoanm.github.io/chessx`         |
...
3. **Self-documenting identifiers** — `hideRatings()` needs no comment;
   `processData(x)` does.
...
2. Stay scoped to `*://*.chess.com/*` — never widen the content-script match
   without a roadmap item.
3. Content scripts must be idempotent — running twice must not corrupt state.
4. Hide elements, never remove them — chess.com state must remain intact.
5. Never add remote resources or network-backed behavior; the extension is
   fully offline.
6. Prefix debug logs with `[ChessX]` and keep them minimal.
...
2. **Manual matrix** — smoke-test after any change:
   ...
   - On chess.com, open a game: the ratings and usernames are hidden and stay
     hidden as the page updates (pairings, live game banners, results)
   - Home/lobby pages still render normally — only the listed classes hide
3. **No false positives** — verify ordinary chess.com navigation is unchanged.

## Before You Push

1. `pnpm lint --filter=@hieudoanm.github.io/chessx`
2. `pnpm format --filter=@hieudoanm.github.io/chessx`
3. `pnpm build --filter=@hieudoanm.github.io/chessx`

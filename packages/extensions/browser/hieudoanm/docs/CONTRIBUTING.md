...
Thanks for contributing to **hieudoanm Extension**, a multi-site browser
extension that tracks Claude.ai usage, detects Shopify stores, hides chess.com
ratings, and extracts YouTube transcripts, on Chromium and Gecko browsers via
both Manifest V2 and Manifest V3 builds.
...
3. **Build this extension**:
   ...
   ```bash
   pnpm build --filter=@hieudoanm.github.io/extension
   ```
...
| Task    | Command                                                   |
| ------- | --------------------------------------------------------- |
| Build   | `pnpm build --filter=@hieudoanm.github.io/extension`      |
| Lint    | `pnpm lint --filter=@hieudoanm.github.io/extension`       |
| Format  | `pnpm format --filter=@hieudoanm.github.io/extension`     |
| Web-ext | `pnpm web-ext lint --source-dir dist/v3` (also `dist/v2`) |
| Clean   | `pnpm clean --filter=@hieudoanm.github.io/extension`      |
...
4. **DRY** — when a pattern repeats, centralize it. Duplication is how bugs get
   missed.
5. **Small, focused files** — functions ≤ 30 lines, files ≤ 200 lines. Each
   feature keeps its own content/background module.
...
2. Feature modules stay scoped to their origin — the Claude fetch patch must
   only intercept `/rate_limits` and `/usage`; non-matching responses pass
   through untouched.
3. Content scripts must be idempotent — running twice must not duplicate the
   indicator widget or double-apply hiding.
4. Badge state lives in `chrome.storage.local`; content-side caches live in
   `localStorage` — keep the two separation of concerns.
5. Never add remote resources or data collection beyond the feature's purpose;
   the extension stays offline and transparent.
6. Prefix debug logs with `[hieudoanm]` and keep them minimal.
...
2. **Manual matrix** — smoke-test after any change:
   ...
   - Claude.ai: open a conversation → inline indicator appears with
     daily/weekly %, badge color matches <60 / 60–89 / ≥90
   - Shopify: load a Shopify store → `CHECK_SHOPIFY` reply returns detected
   - chess.com: open a game → ratings/username hidden, hide re-applies on new
     elements
   - YouTube: request a transcript → caption text returns
3. **No false positives** — the fetch patch must not alter unrelated requests;
   non-Shopify pages reply `false`.

## Before You Push

1. `pnpm lint --filter=@hieudoanm.github.io/extension`
2. `pnpm format --filter=@hieudoanm.github.io/extension`
3. `pnpm build --filter=@hieudoanm.github.io/extension`

...
Thanks for contributing to **GitHubX**, a cross-browser extension that opens
external (non-github.com) links in a new tab from GitHub pages, on Chromium and
...
3. **Build this extension**:
   ...
   ```bash
   pnpm build --filter=@hieudoanm.github.io/githubx
   ```
...
| Task     | Command                                                        |
| -------- | -------------------------------------------------------------- |
| Build    | `pnpm build --filter=@hieudoanm.github.io/githubx`             |
| Lint     | `pnpm lint --filter=@hieudoanm.github.io/githubx`              |
| Format   | `pnpm format --filter=@hieudoanm.github.io/githubx`            |
| Web-ext  | `pnpm web-ext lint --source-dir dist/v3` (also `dist/v2`)      |
| Clean    | `pnpm clean --filter=@hieudoanm.github.io/githubx`             |
...
3. **Self-documenting identifiers** — `isGitHubUrl()` needs no comment;
   `processData(x)` does.
...
5. **Small, focused files** — functions ≤ 30 lines, files ≤ 200 lines.
   `content.ts` deliberately stays tiny.
...
2. Stay scoped to `*://github.com/*` — never widen the content-script match
   without a roadmap item.
3. Never open tabs for a URL that is already a new-tab target (avoid double
   opens); guard against modifier-key duplication.
4. Content scripts must be idempotent — running twice must not corrupt state.
5. Never add remote resources or network-backed behavior; the extension is
   fully offline.
6. Prefix debug logs with `[GitHubX]` and keep them minimal.
...
2. **Manual matrix** — smoke-test after any change:
   ...
   - On a GitHub page, click an external link (e.g. a `github.io` or docs
     site) → opens in a new tab, current page unchanged
   - Relative links (`/owner/repo`) → still navigate normally in the same tab
   - `#section` anchors → no new tab
   - `Ctrl/Cmd+click` on an internal link → native behavior intact
3. **No false positives** — verify ordinary GitHub navigation is unchanged.

## Before You Push

1. `pnpm lint --filter=@hieudoanm.github.io/githubx`
2. `pnpm format --filter=@hieudoanm.github.io/githubx`
3. `pnpm build --filter=@hieudoanm.github.io/githubx`

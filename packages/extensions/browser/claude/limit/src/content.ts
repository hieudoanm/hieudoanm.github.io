 ...
 * content.ts
 *
 * 1. Intercepts fetch() on claude.ai to capture usage data from API responses.
 * 2. Renders a small indicator near the chat input bar showing:
 *      D 50% ↻ 3h 42m  ·  W 80% ↻ 2d 1h
 *
 * No popup. No background script needed — everything lives here.
...
// ─── Mount: find the input bar's bottom toolbar ───────────────────────────────

function findMountTarget(): Element | null {
  // 1. Explicit test ids Claude uses
  const byId = document.querySelector('[data-testid="input-menu-container"]');
  if (byId) return byId;

  // 2. Bottom action row near the Send button
  const sendBtn =
  ...
  // 3. ProseMirror editor's ancestor form/fieldset
...
function renderIndicator(data: LimitData) {
  ...
  if (!target) return;
  // Prepend so it sits at the left end of the toolbar row
...
// Re-attach when Claude re-renders the page (SPA navigation)
...
// Tick the reset countdown every minute
setInterval(() => {
  const data = loadData();

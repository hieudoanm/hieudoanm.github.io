# Ads Blocker — Packaging Checklist

Checklist for shipping Ads Blocker to users as a loadable extension: MV2/MV3
artifact builds, store submission (Chrome Web Store, Edge Add-ons, Mozilla
AMO), and the `web-ext` lint gate.

App: `ads-blocker`, version 0.0.1. Manifest IDs:
`ads-blocker-extension@hieudoanm.github.io` (Gecko).

## Common

- [ ] Bump `version` in `package.json` and both `public/manifest/*/manifest.json`
      in sync
- [ ] Run `pnpm build --filter=@hieudoanm.github.io/ads-blocker` and confirm
      `dist/v2` and `dist/v3` both emit `background.js`, `content.js`, and
      `manifest.json` (plus `rules.json` for v3)
...
- [ ] Release assets uploaded under the tag
      `extensions-browser-ads-blocker-latest` with the naming scheme
      `ads-blocker-v{2|3}.{crx,xpi,zip}`
...
- [ ] Wide usage permission (`<all_urls>` `host_permissions` /
      `declarativeNetRequest`) justified in the permission rationale
...
- [ ] `browser_specific_settings.gecko.id` matches
      `ads-blocker-extension@hieudoanm.github.io`
...
- [ ] Answer review questions: no remote code, no data collection, permissions
      limited to `<all_urls>` + `webRequest`

## Release Automation

- [ ] Tag a release `extensions-browser-ads-blocker-<version>` on GitHub

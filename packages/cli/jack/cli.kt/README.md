...
```bash
jack calc bmi 70 175          # BMI calc (metric)
jack casino poker odds "Ah Kh" --simulations 1000
jack casino baccarat strategy --simulations 50000
jack colors convert-hex "#ff6600" --to hsl
jack convert base64 encode "hello world"
jack crypto hash sha256 "data"
jack doi fetch 10.1000/xyz123
jack file read README.md --lines 10
jack net ping example.com
jack search files --pattern "*.kt"
jack semver bump --minor --version 1.2.3
jack time world --zone Asia/Tokyo
jack version
```
...
```bash
jack gemini code "write a fibonacci function"
jack openrouter models                    # list models
jack openrouter code "explain monads"     # chat via OpenRouter
```

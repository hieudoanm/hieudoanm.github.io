...
fn parse_host(host: &str) -> (String, String) {
    ...
    (host_port, server_name)
}

fn openssl_x509(host: &str, args: &[&str]) -> anyhow::Result<String> {
    let (host_port, server_name) = parse_host(host);
...
fn check_cert_status(not_after: &str, warn_days: i64) -> String {
...
mod tests {
    ...
    fn test_parse_host_with_port() {
        let (host_port, server_name) = parse_host("example.com:8443");
        assert_eq!(host_port, "example.com:8443");
        assert_eq!(server_name, "example.com");
    }

    #[test]
    fn test_parse_host_without_port() {
        let (host_port, server_name) = parse_host("example.com");
    ...
    fn test_parse_host_empty() {
        let (host_port, server_name) = parse_host("");
        assert_eq!(host_port, ":443");
    ...
    #[test]
    fn test_check_cert_status_empty() {
        assert_eq!(check_cert_status("", 30), "unknown");
    }

    #[test]
    fn test_check_cert_status_parse_fail() {
        assert_eq!(check_cert_status("bad-date-format", 30), "Expires: bad-date-format");
    }
}
...
pub async fn run(matches: &clap::ArgMatches) -> anyhow::Result<()> {
    match matches.subcommand() {
        ...
        Some(("check", m)) => {
            ...
            let status = check_cert_status(&not_after, warn_days);

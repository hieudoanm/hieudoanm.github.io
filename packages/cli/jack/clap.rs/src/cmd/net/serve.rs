...
mod tests {
    ...
    #[test]
    fn test_write_response_headers_200() {
        let mut resp = Vec::new();
        write_response_headers(&mut resp, 200, "text/plain", 12, false);
        let output = String::from_utf8(resp).unwrap();
    ...
    #[test]
    fn test_write_response_headers_404() {
        let mut resp = Vec::new();
        write_response_headers(&mut resp, 404, "text/html", 0, false);
        let output = String::from_utf8(resp).unwrap();
    ...
    #[test]
    fn test_write_response_headers_500() {
        let mut resp = Vec::new();
        write_response_headers(&mut resp, 500, "application/json", 5, false);
        let output = String::from_utf8(resp).unwrap();
    ...
    #[test]
    fn test_write_response_headers_405() {
        let mut resp = Vec::new();
        write_response_headers(&mut resp, 405, "text/plain", 0, false);
        let output = String::from_utf8(resp).unwrap();
        assert!(output.contains("405 Method Not Allowed"));
    }

    #[test]
    fn test_write_response_headers_204() {
        let mut resp = Vec::new();
        write_response_headers(&mut resp, 204, "text/plain", 0, false);
        let output = String::from_utf8(resp).unwrap();
        assert!(output.contains("204 No Content"));
    }

    #[test]
    fn test_write_response_headers_unknown_status() {
        let mut resp = Vec::new();
        write_response_headers(&mut resp, 999, "text/plain", 0, false);
        let output = String::from_utf8(resp).unwrap();
        assert!(output.contains("999 Unknown"));
    }

    #[test]
    fn test_write_response_headers_with_cors() {
        let mut resp = Vec::new();
        write_response_headers(&mut resp, 200, "application/json", 4, true);
        let output = String::from_utf8(resp).unwrap();
        assert!(output.contains("200 OK"));
        assert!(output.contains("Access-Control-Allow-Origin: *"));
        assert!(output.contains("Access-Control-Allow-Methods: GET, OPTIONS"));
    }
}

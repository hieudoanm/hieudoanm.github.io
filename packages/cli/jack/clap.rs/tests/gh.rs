...
fn test_dir(name: &str) -> String {
    ...
    let dir = format!("/tmp/jack_test_gh_{name}_{id}");
...
fn gh_languages_invalid_format() {
    ...
    assert!(stderr.contains("owner/repo"), "stderr: {stderr}");
}
...
fn gh_ignore_with_name() {
    ...
    assert!(Path::new(&out).exists(), ".gitignore file should exist");
    let content = fs::read_to_string(&out).unwrap_or_default();
    assert!(

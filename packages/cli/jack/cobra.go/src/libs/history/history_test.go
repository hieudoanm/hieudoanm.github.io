...
func TestStoragePath_MkdirAllError(t *testing.T) {
	...
	t.Setenv("HOME", tmpDir)
	t.Setenv("USERPROFILE", tmpDir)
	// Create .jack as a file so MkdirAll fails
	os.WriteFile(filepath.Join(tmpDir, ".jack"), []byte("x"), 0644)
	_, err := storagePath()
	if err == nil {
		t.Error("expected error when .jack is a file")
	}
}
...
func TestClear_RemoveError(t *testing.T) {
	...
	// Make .jack directory read-only so Remove fails
	dir := filepath.Join(tmpDir, ".jack")
	os.Chmod(dir, 0555)
...
func TestReadAllCorruptedLines(t *testing.T) {
	baseDir := t.TempDir()
	dir := filepath.Join(baseDir, ".jack")

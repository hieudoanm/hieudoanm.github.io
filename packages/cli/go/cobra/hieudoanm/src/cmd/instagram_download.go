  ...
  devtools instagram download CLI7qRNhI_o --index 1`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if imgIndex > 0 {
			...
			imageURLs = []quality_mock_urls{imageURLs[imgIndex-1]} // reuse variable name for simplicity
...
type quality_mock_urls = string // helper for index logic

func init() {
	instagramCmd.AddCommand(instagramDownloadCmd)

	instagramDownloadCmd.Flags().StringP("output", "o", ".", "Output directory")
	instagramDownloadCmd.Flags().IntP("index", "i", 0, "Specific image index to download (1-based)")
	instagramDownloadCmd.Flags().BoolP("proxy", "p", false, "Use proxy to fetch content")
}

...
func init() {
	youtubeCmd.AddCommand(thumbnailsCmd)

	thumbnailsCmd.Flags().StringP("quality", "q", "", fmt.Sprintf("specific quality to download (%s)", validQualityIDs()))
	thumbnailsCmd.Flags().StringP("output", "o", ".", "output directory")
	thumbnailsCmd.Flags().BoolP("all", "a", false, "download all quality variants")
	thumbnailsCmd.Flags().BoolP("list", "l", false, "list thumbnail URLs without downloading")
}

...
pub async fn run_write(matches: &Args, write: &mut impl Write) -> anyhow::Result<()> {
    ...
    if json {
        ...
        writeln!(write, "=== Loan Amortization ===")?;
        writeln!(write, "Principal:     {:15.2}", principal)?;
        writeln!(write, "Annual rate:   {:15.2}%", rate)?;
        writeln!(write, "Years:         {:15.0}", years)?;
        writeln!(write, "Monthly:       {:15.2}", payment)?;
        writeln!(write, "Total paid:    {:15.2}", total_payment)?;
        writeln!(write, "Total interest:{:15.2}", total_interest)?;
        writeln!(write)?;
        writeln!(write, "{:<6} {:>12} {:>12} {:>12}", "Month", "Payment", "Interest", "Balance")?;
        writeln!(write, "------   ------------   ------------   ------------")?;
        let mut balance = principal;
        for i in 1..=n.min(12) {
            let interest = balance * r;
            let principal_paid = payment - interest;
            balance -= principal_paid;
            writeln!(write, "{:<6} {:>12.2} {:>12.2} {:>12.2}", i, payment, interest, balance.max(0.0))?;
        }
        if n > 12 {
            writeln!(write, "... ({} more months)", n - 12)?;
        }
    }
...
mod tests {
    ...
    async fn test_run_write_json() {
        ...
        run_write(&args, &mut buf).await.unwrap();
        let output = String::from_utf8(buf).unwrap();
        assert!(output.contains("\"monthly\": 188.71"));
    }

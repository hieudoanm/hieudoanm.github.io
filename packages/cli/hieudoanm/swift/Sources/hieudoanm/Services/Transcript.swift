...
func fetchTranscript(videoID: String, language: String = "en") async throws -> Transcript {
    let innertubeURL = "https://www.youtube.com/watch?v=\(videoID)"
    var options = RequestsOptions()
    options.headers["User-Agent"] = "Mozilla/5.0"
    options.headers["Accept-Language"] = "en-US,en;q=0.5"

    let html = try await requestsFetch(method: "GET", url: innertubeURL, options: options)
    let htmlStr = String(data: html, encoding: .utf8) ?? ""

    guard let captionData = extractCaptionJson(from: htmlStr) else {
    ...
    let xmlData = try await requestsFetch(method: "GET", url: selectedTrack)
    return try parseTimedText(String(data: xmlData, encoding: .utf8) ?? "")
}

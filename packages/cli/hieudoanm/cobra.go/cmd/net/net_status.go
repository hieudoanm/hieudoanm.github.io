...
func fetchStatuses(services []ServiceStatus) tea.Cmd {
	return func() tea.Msg {
		...
		s := services[0]
		out := GetDescriptiveStatus(s.Name, s.URL, statusDebug)
...
func (m statusModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.WindowSizeMsg:
		...
		m.viewport.SetContent(m.renderSRows())
		return m, nil
...
func renderSRow(name, status string, width int) string {
	left := name
...
func (m statusModel) renderSRows() string {
	var b strings.Builder
	for i, svc := range m.services {
		row := renderSRow(svc.Name, svc.Output, m.viewport.Width())
		if i == m.cursor {

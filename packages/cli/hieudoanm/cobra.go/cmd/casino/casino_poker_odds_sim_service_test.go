...
func TestRemoveCardsNoRemovals(t *testing.T) {
	deck := newPokerDeck()
	result := removeCards(deck, map[Card]bool{})
	if len(result) != 52 {
...
func TestToCardMap(t *testing.T) {
	cards := []Card{{2, Clubs}, {14, Spades}, {7, Diamonds}}
	m := toCardMap(cards)
	if len(m) != 3 {
...
func TestFormatCards(t *testing.T) {
	...
	// sorted by rank desc: Ah(14) then Kd(13)
	want := []Card{{14, Hearts}, {13, Diamonds}}
	for i := range want {
		if result[i] != want[i] {
...
func TestFormatCardsSortsByRankThenSuit(t *testing.T) {
	...
	// same rank(14), sort by suit asc: Clubs(0) < Spades(3)
	want := []Card{{14, Clubs}, {14, Spades}}
	for i := range want {
		if result[i] != want[i] {
			t.Errorf("result[%d] = %v, want %v", i, result[i], want[i])
		}
...
func TestFormatCardsSingle(t *testing.T) {
	...
	if result[0] != (Card{Rank: 2, Suit: Clubs}) {

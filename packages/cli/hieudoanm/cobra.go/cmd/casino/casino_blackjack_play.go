...
func renderCards(cards []Card, hide bool) string {
	var faces []string
	for i, c := range cards {
		if hide && i == 0 {
			faces = append(faces, "🂠")
...
func runDealer(m *playModel) {
	for handValue(m.dealer) < 17 {
		m.dealer = append(m.dealer, dealCard(&m.deck))
	}
}
...
func (m playModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case playBet:
			...
			case "enter":
				if m.bet > 0 && m.bet <= m.balance {
					...
					m.player = []Card{dealCard(&m.deck), dealCard(&m.deck)}
					m.dealer = []Card{dealCard(&m.deck), dealCard(&m.deck)}
					pv := handValue(m.player)
					dv := handValue(m.dealer)
					if pv == 21 && dv == 21 {
						m.balance += m.bet
		...
		case playPlayer:
			switch msg.String() {
			case "h":
				m.player = append(m.player, dealCard(&m.deck))

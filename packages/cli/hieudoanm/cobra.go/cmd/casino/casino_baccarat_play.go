...
type baccaratModel struct {
	...
	balance   int
	bet       int
	betType   string // player, banker, tie
	state     baccaratState
	message   string
	result    string
	pValue    int
...
func baccaratShouldDraw(cards []Card) bool {
	v := baccaratSum(cards)
	return v <= 5
}

func baccaratDrawForThird(cards []Card, playerThird int) bool {
	v := baccaratSum(cards)
	switch {
	case v <= 2:
		return true
...
func (m *baccaratModel) deal() {
	m.player = []Card{m.deck[0], m.deck[1]}
	m.banker = []Card{m.deck[2], m.deck[3]}
	m.deck = m.deck[4:]

	m.pValue = baccaratSum(m.player)
	m.bValue = baccaratSum(m.banker)
	...
	if baccaratShouldDraw(m.player) {
		m.player = append(m.player, m.deck[0])
		m.deck = m.deck[1:]
		m.pValue = baccaratSum(m.player)
		pc := m.player[2]
		pVal := baccaratValue(pc)
		if baccaratDrawForThird(m.banker, pVal) {
			m.banker = append(m.banker, m.deck[0])
			m.deck = m.deck[1:]
			m.bValue = baccaratSum(m.banker)
		}
	} else {
		if baccaratShouldDraw(m.banker) {
			m.banker = append(m.banker, m.deck[0])
			m.deck = m.deck[1:]
			m.bValue = baccaratSum(m.banker)
		}
...
func (m *baccaratModel) finish() {
	pv := baccaratSum(m.player)
	bv := baccaratSum(m.banker)
...
func (m baccaratModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case baccaratBet:
			switch msg.String() {
			case "up":
				m.bet = minInt(m.bet+25, m.balance)
			case "down":
				if m.bet >= 25 {
					m.bet -= 25
				}
...
func (m baccaratModel) View() tea.View {
	...
	case baccaratResult:
		pc := cardsDisplay(m.player)
		bc := cardsDisplay(m.banker)
		...
		} else if strings.Contains(m.result, "lose") || strings.Contains(m.result, "lose") {

...
func NewCommand() *cobra.Command {
  ...
  epoch --iso`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if relative != "" {
				t, err := parseRelative(relative)
				if err != nil {
					return err
				}
				if json {
					printJSON(t.Unix(), t.Format(time.RFC3339))
				} else {
					fmt.Println(t.Unix())
				}
				return nil
			...
			if len(args) == 0 {
				now := time.Now()
				if json {
					printJSON(now.Unix(), now.Format(time.RFC3339))
				} else {
			...
			if json {
				printJSON(sec, t.Format(time.RFC3339))
				return nil
			}
...
func printJSON(epoch int64, rfc3339 string) {

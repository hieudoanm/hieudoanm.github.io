...
func baccaratValue(c internal.Card) int {
	if c.Rank >= 10 {
		return 0
	}
	return c.Rank
...
func baccaratSum(cards []internal.Card) int {
	var s int
	for _, c := range cards {
		s += baccaratValue(c)
	}
...
func baccaratShouldDraw(cards []internal.Card) bool {
	v := baccaratSum(cards)
	return v <= 5
}

func baccaratDrawForThird(cards []internal.Card, playerThird int) bool {
	v := baccaratSum(cards)
	switch {
	case v <= 2:
		return true

...
var (
	sysCPUPercent     = cpu.Percent
	sysMemVirtual     = mem.VirtualMemory
	sysDiskIOCounters = disk.IOCounters
	sysNetIOCounters  = net.IOCounters
	sysHostUptime     = host.Uptime
	sysProcesses      = process.Processes
)

func sysGatherMetrics() (SysMetrics, error) {
	var m SysMetrics

	if p, _ := sysCPUPercent(0, false); len(p) > 0 {
		m.CPUTotal = p[0]
	}
	m.CPUPerCore, _ = sysCPUPercent(0, true)

	if vm, err := sysMemVirtual(); err == nil {
		m.RAMUsedGB = float64(vm.Used) / 1e9
		m.RAMTotalGB = float64(vm.Total) / 1e9
		m.RAMPct = vm.UsedPercent
	}

	if ioMap, err := sysDiskIOCounters(); err == nil {
		for _, d := range ioMap {
	...
	if netStats, _ := sysNetIOCounters(false); len(netStats) > 0 {
		m.NetRxKB = netStats[0].BytesRecv / 1024
		m.NetTxKB = netStats[0].BytesSent / 1024
	}

	m.Uptime, _ = sysHostUptime()

	procs, _ := sysProcesses()
	for _, p := range procs {
		name, _ := p.Name()

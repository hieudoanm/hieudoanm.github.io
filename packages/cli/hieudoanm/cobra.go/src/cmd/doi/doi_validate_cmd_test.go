...
func TestNewValidateCmd_RunE_ValidDOI(t *testing.T) {
	cmd := newValidateCmd()
...
func TestNewValidateCmd_RunE_InvalidDOI(t *testing.T) {
	cmd := newValidateCmd()
	output := captureStdout(func() {
...
func TestNewValidateCmd_RunE_NoArgs(t *testing.T) {
	cmd := newValidateCmd()
	output := captureStdout(func() {
...
func TestNewValidateCmd_RunE_EmptyString(t *testing.T) {
	cmd := newValidateCmd()
	output := captureStdout(func() {
		if err := cmd.RunE(cmd, []string{""}); err != nil {
			t.Fatal(err)

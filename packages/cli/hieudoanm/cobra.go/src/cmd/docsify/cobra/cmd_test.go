...
`
	...
	cmd.SetArgs([]string{"--file", dir, "--output", outPath})
	err := cmd.Execute()
	if err == nil {
		t.Error("expected error for directory with no cobra commands")
	}
}
...
func TestNewCmd_RunE_BadOutputPath(t *testing.T) {
	cmd := NewCmd()
	cmd.SetArgs([]string{"--output", "/nonexistent-deep-path/output.md"})
	err := cmd.Execute()
	if err == nil {

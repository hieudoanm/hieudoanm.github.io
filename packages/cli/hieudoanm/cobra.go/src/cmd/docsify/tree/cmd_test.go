...
func TestNewCmd_RunE_Error(t *testing.T) {
	cmd := NewCmd()
	cmd.SetArgs([]string{"--dir", "/nonexistent-test-path", "--out", filepath.Join(t.TempDir(), "tree.md")})
	err := cmd.Execute()
	if err == nil {
		t.Error("expected error for nonexistent directory")
	}
...
func TestNewCmd_RunE_BadOutput(t *testing.T) {
	cmd := NewCmd()
	cmd.SetArgs([]string{"--out", "/nonexistent-test-path/deep/tree.md"})
	err := cmd.Execute()
	if err == nil {

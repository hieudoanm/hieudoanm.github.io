...
func TestGetCompetitions(t *testing.T) {
	...
	t.Run("NewFootballDataClient default", func(t *testing.T) {
		c := NewFootballDataClient("my-token")
		if c.BaseURL != "https://api.football-data.org/v4" {
			t.Errorf("expected https://api.football-data.org/v4, got %s", c.BaseURL)
		}
		if c.AuthToken != "my-token" {
			t.Errorf("expected my-token, got %s", c.AuthToken)
		}
	})
}

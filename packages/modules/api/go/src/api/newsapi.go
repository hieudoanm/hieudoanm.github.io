...
func GetTopHeadlines(apiKey string, req TopHeadlinesRequest) (*ArticleResponse, error) {
	...
	if req.Page > 0 { q.Set("page", strconv.Itoa(req.Page)) }
	if req.PageSize > 0 { q.Set("pageSize", strconv.Itoa(req.PageSize)) }
	if req.Query != "" { q.Set("q", req.Query) }
	if len(req.Sources) > 0 { q.Set("sources", strings.Join(req.Sources, ",")) }
	u.RawQuery = q.Encode()

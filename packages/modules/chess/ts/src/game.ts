...
import { square, rankOf, fileOf } from './utils';
import { stringifyFEN, parseFEN } from './notation';
...
const hasInsufficientMaterial = (board: Board): boolean => {
  ...
  if (pieces.length === 4) {
    ...
    if (bishops.length === 2) {
      ...
      const color0 = (rankOf(sq0) + fileOf(sq0)) % 2;
      const color1 = (rankOf(sq1) + fileOf(sq1)) % 2;
...
export const makeMove = (state: GameState, move: Move): GameState => {
  ...
  const newEnPassant: Square | null =
    piece.type === 'p' && Math.abs(rankOf(move.to) - rankOf(move.from)) === 2
      ? square((rankOf(move.from) + rankOf(move.to)) / 2, fileOf(move.from))
      : null;
  ...
  const stateBeforeFen = stringifyFEN(state);
  ...
  const moves = legalMoves(newBoard, newTurn, newCastlingRights, newEnPassant);
...
export const undoMove = (state: GameState): GameState => {
  ...
  return parseFEN(prev.stateBefore);
};

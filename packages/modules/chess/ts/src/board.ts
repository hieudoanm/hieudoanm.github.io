import type { Board, Piece, Square, PieceType, Color } from './types';
...
export const putPiece = (board: Board, piece: Piece, sq: Square): void => {
  board[sq] = piece;
};
...
export const boardFromFen = (boardPart: string): Board => {
  ...
  for (let r = 0; r < 8; r++) {
    ...
    for (const ch of row) {
      if (ch >= '1' && ch <= '8') {
        ...
        if (piece) board[square(7 - r, f)] = piece;
        f++;
      }
    }
  }

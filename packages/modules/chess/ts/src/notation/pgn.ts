...
import { fromFen, toSan } from './notation';
...
export const fromPgn = (input: string): PGNGame[] => {
  const games: PGNGame[] = [];
...
export const toPgn = (games: PGNGame[]): string => {
  return games
    .map((game) => {
...
export const toPgnFromState = (state: GameState): string => {
  const sanList: string[] = [];
  for (const entry of state.history) {
    const before = fromFen(entry.stateBefore);
    const san = toSan(
      before.board,
      entry.move,
      before.turn,

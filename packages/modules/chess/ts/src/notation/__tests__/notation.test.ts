import { fromFen, toFen, toUci, fromUci, toSan } from '../notation';
...
describe('toFen', () => {
  ...
  it('round-trips after a move', () => {
    ...
    const reparsed = fromFen(toFen(after));
...
describe('toSan', () => {
  ...
  it('castling kingside', () => {
    const game = fromFen('r3k2r/8/8/8/8/8/8/R3K2R w KQkq - 0 1');
    const san = toSan(
      game.board,
  ...
  it('pawn promotion with capture', () => {
    const game = fromFen('8/1kP5/8/8/8/5K2/8/8 w - - 0 1');
    const san = toSan(
      game.board,

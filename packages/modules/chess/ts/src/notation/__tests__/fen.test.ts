...
describe('fromFenFields', () => {
  ...
  it('throws on invalid FEN (wrong field count)', () => {
    expect(() => fromFenFields('invalid fen string')).toThrow('Invalid FEN');
  });

  it('throws on invalid active color', () => {
    expect(() => fromFenFields('8/8/8/8/8/8/8/8 x - - 0 1')).toThrow(
      'active color'
    );
  });
});

describe('toFenFields', () => {
  ...
  it('round-trips parse → stringify → parse', () => {
    ...
    const parsed = fromFenFields(original);
    const stringified = toFenFields(parsed);
    const reparsed = fromFenFields(stringified);

...
import { cloneBoard, getPiece, removePiece, putPiece, findKing } from './board';
...
const applyMoveToBoard = (board: Board, move: Move): void => {
  ...
  if (move.promotion) {
    putPiece(board, { color: piece.color, type: move.promotion }, move.to);
  } else {
    putPiece(board, piece, move.to);
  }
  ...
  if (piece.type === 'k' && fileDiff === 2) {
    const rookFrom =
      fileOf(move.to) > fileOf(move.from) ? move.to + 1 : move.to - 2;
    const rookTo =
      fileOf(move.to) > fileOf(move.from) ? move.to - 1 : move.to + 1;
    const rank = rankOf(move.to);
    const rook = getPiece(board, square(rank, rookFrom));
    if (rook) {
      removePiece(board, square(rank, rookFrom));
      putPiece(board, rook, square(rank, rookTo));
    }
  }

  if (piece.type === 'p' && fileDiff !== 0 && !move.captured) {
    const epRank = rankOf(move.from);
    const epFile = fileOf(move.to);
    removePiece(board, square(epRank, epFile));
  }
};
...
export const generatePawnMoves = (
  ...
  const f1 = square(rank + dir, file);
  if (isValidSquare(f1) && !board[f1]) {
    ...
    if (rank === startRank) {
      const f2 = square(rank + 2 * dir, file);
      if (!board[f2]) {
  ...
  for (const df of [-1, 1]) {
    ...
    if (to === enPassant) {
      moves.push({
        ...
        to,
        promotion: null,
        captured: board[square(rank, f)] ?? null,
      });
    }
  }
...
export const generateKnightMoves = (
  ...
  for (const offset of offsets) {
    const to = sq + offset;
    if (!isValidSquare(to)) continue;
    const rDiff = Math.abs(rankOf(to) - rank);
    const fDiff = Math.abs(fileOf(to) - file);
...
export const generateKingMoves = (
  ...
  if (castlingRights[castlingKey as keyof CastlingRights]) {
    const kingDest = square(startRank, 6);
    const rookSq = square(startRank, 7);
    const rook = board[rookSq];
    if (
      rook &&
      rook.type === 'r' &&
      rook.color === color &&
      !board[square(startRank, 5)] &&
      !board[square(startRank, 6)] &&
      !isSquareAttacked(board, sq, opponent) &&
      !isSquareAttacked(board, square(startRank, 5), opponent) &&
      !isSquareAttacked(board, square(startRank, 6), opponent)
    ) {
  ...
  if (castlingRights[castlingQueenKey as keyof CastlingRights]) {
    const kingDest = square(startRank, 2);
    const rookSq = square(startRank, 0);
    const rook = board[rookSq];
    if (
      rook &&
      rook.type === 'r' &&
      rook.color === color &&
      !board[square(startRank, 1)] &&
      !board[square(startRank, 2)] &&
      !board[square(startRank, 3)] &&
      !isSquareAttacked(board, sq, opponent) &&
      !isSquareAttacked(board, square(startRank, 3), opponent) &&
      !isSquareAttacked(board, square(startRank, 2), opponent)
    ) {
      moves.push({ from: sq, to: kingDest, promotion: null, captured: null });
    }
  }
...
export const generatePseudoLegalMoves = (
  ...
  for (let sq = 0; sq < 64; sq++) {
    ...
    switch (piece.type) {
      case 'p':
        moves.push(...generatePawnMoves(board, sq, turn, enPassant));
        break;
      case 'n':
        moves.push(...generateKnightMoves(board, sq, turn));
        break;
      case 'b':
        moves.push(...generateSlidingMoves(board, sq, turn, [-9, -7, 7, 9]));
        break;
      case 'r':
        moves.push(...generateSlidingMoves(board, sq, turn, [-8, -1, 1, 8]));
        break;
      case 'q':
        moves.push(
          ...generateSlidingMoves(board, sq, turn, [-9, -7, -8, -1, 1, 7, 8, 9])
        );
        break;
      case 'k':
        moves.push(...generateKingMoves(board, sq, turn, castlingRights));
        break;
    }
  }
  return moves;

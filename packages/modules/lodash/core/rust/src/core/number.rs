...
mod tests {
    ...
    #[test]
    fn test_random_auto_float() {
        let r = random(1.5, Some(2.5), None);
        assert!(r >= 1.5 && r <= 2.5);
    }

    #[test]
    fn test_random_auto_int() {
        for _ in 0..100 {
            let r = random(5.0, Some(10.0), None);
            assert!(r >= 5.0 && r <= 10.0);
        }
    }
    ...
    #[test]
    fn test_in_range_negative_start() {
        assert!(in_range(-3.0, -5.0, Some(-1.0)));
        assert!(!in_range(-6.0, -5.0, Some(-1.0)));
        assert!(!in_range(0.0, -5.0, Some(-1.0)));
    }

    #[test]
    fn test_in_range_none_negative() {
        assert!(in_range(-3.0, -5.0, None));
        assert!(!in_range(1.0, -5.0, None));
        assert!(in_range(-5.0, -5.0, None)); // lower bound is inclusive
    }
}

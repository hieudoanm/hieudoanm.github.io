...
mod tests {
    ...
    fn test_escape_regexp() {
        assert_eq!(escape_regexp(".*+?^${}()|[]\\"), "\\.\\*\\+\\?\\^\\$\\{\\}\\(\\)\\|\\[\\]\\\\");
        assert_eq!(escape_regexp("hello"), "hello");
    }
    ...
    #[test]
    fn test_pad_end() {
        assert_eq!(pad_end("abc", 6, "*"), "abc***");
        assert_eq!(pad_end("abc", 3, "*"), "abc");
    }

    #[test]
    fn test_pad_start() {
        assert_eq!(pad_start("abc", 6, "*"), "*abc***");
        assert_eq!(pad_start("abc", 3, "*"), "abc");
    }

    #[test]
    fn test_parse_int() {
        assert_eq!(parse_int("08", 8), 0);
        assert_eq!(parse_int("10", 2), 2);
        assert_eq!(parse_int("-10", 10), -10);
        assert_eq!(parse_int("+10", 10), 10);
        assert_eq!(parse_int("abc", 10), 0);
        assert_eq!(parse_int("10", 1), 0);
        assert_eq!(parse_int("10", 37), 0);
    }
    ...
    fn test_split() {
        ...
        assert_eq!(split("a-b-c", "-", 0), Vec::<&str>::new());
        assert_eq!(split("a-b-c", "-", 5), vec!["a", "b", "c"]);
    }

    #[test]
    fn test_start_case() {
        assert_eq!(start_case("fooBar"), "Foo Bar");
        assert_eq!(start_case("--foo-bar--"), "Foo Bar");
    }
    ...
    fn test_template() {
        ...
        assert_eq!(template("Hello {{name}}!", &data), "Hello world!");
        assert_eq!(template("No templates", &data), "No templates");
    }
    ...
    #[test]
    fn test_upper_case() {
        assert_eq!(upper_case("Foo Bar"), "FOO BAR");
        assert_eq!(upper_case("fooBar"), "FOO BAR");
    }

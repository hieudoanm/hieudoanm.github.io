...
mod tests {
    ...
    #[test]
    fn test_wasm_lunar_calendar_new() {
        let cal = WasmLunarCalendar::new();
        assert!(cal.leap_days(2024) >= 0);
    }

    #[test]
    fn test_wasm_lunar_calendar_leap_month() {
        let cal = WasmLunarCalendar::new();
        assert_eq!(cal.leap_month(2023), 2);
    }

    #[test]
    fn test_wasm_lunar_calendar_l_year_days() {
        let cal = WasmLunarCalendar::new();
        assert!(cal.l_year_days(2024) > 0);
    }

    #[test]
    fn test_wasm_lunar_calendar_month_days() {
        let cal = WasmLunarCalendar::new();
        let days = cal.month_days(2024, 1);
        assert!(days == 29 || days == 30);
        assert_eq!(cal.month_days(2024, 13), -1);
    }
}

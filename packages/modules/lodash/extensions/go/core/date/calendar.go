package date

import (
	"fmt"
	"time"
)

type CurrentMonth int

const (
	CurrentMonthPrevious CurrentMonth = iota
	CurrentMonthCurrent
	CurrentMonthNext
)

type CalendarDay struct {
	Date         int
	CurrentMonth CurrentMonth
}

func Calendar(year, month int) [][]CalendarDay {
	first := time.Date(year, time.Month(month), 1, 0, 0, 0, 0, time.UTC)
	firstDay := int(first.Weekday())
	mDays := monthDays(year, month)
	prevMonthDays := 0
	if month == 1 {
		prevMonthDays = monthDays(year-1, 12)
	} else {
		prevMonthDays = monthDays(year, month-1)
	}

	var weeks [][]CalendarDay
	dateCounter := 1
	for i := 0; i < 6; i++ {
		var week []CalendarDay
		for j := 0; j < 7; j++ {
			var day CalendarDay
			if i == 0 && j < firstDay {
				day = CalendarDay{
					Date:         prevMonthDays - firstDay + j + 1,
					CurrentMonth: CurrentMonthPrevious,
				}
			} else if dateCounter <= mDays {
				day = CalendarDay{Date: dateCounter, CurrentMonth: CurrentMonthCurrent}
				dateCounter++
			} else {
				day = CalendarDay{Date: dateCounter - mDays, CurrentMonth: CurrentMonthNext}
				dateCounter++
			}
			week = append(week, day)
		}
		weeks = append(weeks, week)
		if dateCounter > mDays && i > 3 {
			break
		}
	}
	return weeks
}

func monthDays(year, month int) int {
	nextMonth := month + 1
	nextYear := year
	if nextMonth > 12 {
		nextMonth = 1
		nextYear++
	}
	first := time.Date(year, time.Month(month), 1, 0, 0, 0, 0, time.UTC)
	next := time.Date(nextYear, time.Month(nextMonth), 1, 0, 0, 0, 0, time.UTC)
	return int(next.Sub(first).Hours() / 24)
}

func WeekOfYear(d time.Time) int {
	_, week := d.ISOWeek()
	return week
}

// ---------------------------------------------------------------------------
// LunarCalendar
// ---------------------------------------------------------------------------

type LunarCalendar struct{}

var lunarInfo = [201]int64{
	0x04bd8, 0x04ae0, 0x0a570, 0x054d5, 0x0d260, 0x0d950, 0x16554, 0x056a0,
	0x09ad0, 0x055d2, 0x04ae0, 0x0a5b6, 0x0a4d0, 0x0d250, 0x1d255, 0x0b540,
	0x0d6a0, 0x0ada2, 0x095b0, 0x14977, 0x04970, 0x0a4b0, 0x0b4b5, 0x06a50,
	0x06d40, 0x1ab54, 0x02b60, 0x09570, 0x052f2, 0x04970, 0x06566, 0x0d4a0,
	0x0ea50, 0x06e95, 0x05ad0, 0x02b60, 0x186e3, 0x092e0, 0x1c8d7, 0x0c950,
	0x0d4a0, 0x1d8a6, 0x0b550, 0x056a0, 0x1a5b4, 0x025d0, 0x092d0, 0x0d2b2,
	0x0a950, 0x0b557, 0x06ca0, 0x0b550, 0x15355, 0x04da0, 0x0a5b0, 0x14573,
	0x052b0, 0x0a9a8, 0x0e950, 0x06aa0, 0x0aea6, 0x0ab50, 0x04b60, 0x0aae4,
	0x0a570, 0x05260, 0x0f263, 0x0d950, 0x05b57, 0x056a0, 0x096d0, 0x04dd5,
	0x04ad0, 0x0a4d0, 0x0d4d4, 0x0d250, 0x0d558, 0x0b540, 0x0b6a0, 0x195a6,
	0x095b0, 0x049b0, 0x0a974, 0x0a4b0, 0x0b27a, 0x06a50, 0x06d40, 0x0af46,
	0x0ab60, 0x09570, 0x04af5, 0x04970, 0x064b0, 0x074a3, 0x0ea50, 0x06b58,
	0x05ac0, 0x0ab60, 0x096d5, 0x092e0, 0x0c960, 0x0d954, 0x0d4a0, 0x0da50,
	0x07552, 0x056a0, 0x0abb7, 0x025d0, 0x092d0, 0x0cab5, 0x0a950, 0x0b4a0,
	0x0baa4, 0x0ad50, 0x055d9, 0x04ba0, 0x0a5b0, 0x15176, 0x052b0, 0x0a930,
	0x07954, 0x06aa0, 0x0ad50, 0x05b52, 0x04b60, 0x0a6e6, 0x0a4e0, 0x0d260,
	0x0ea65, 0x0d530, 0x05aa0, 0x076a3, 0x096d0, 0x04afb, 0x04ad0, 0x0a4d0,
	0x1d0b6, 0x0d250, 0x0d520, 0x0dd45, 0x0b5a0, 0x056d0, 0x055b2, 0x049b0,
	0x0a577, 0x0a4b0, 0x0aa50, 0x1b255, 0x06d20, 0x0ada0, 0x14b63, 0x09370,
	0x049f8, 0x04970, 0x064b0, 0x168a6, 0x0ea50, 0x06b20, 0x1a6c4, 0x0aae0,
	0x0a2e0, 0x0d2e3, 0x0c960, 0x0d557, 0x0d4a0, 0x0da50, 0x05d55, 0x056a0,
	0x0a6d0, 0x055d4, 0x052d0, 0x0a9b8, 0x0a950, 0x0b4a0, 0x0b6a6, 0x0ad50,
	0x055a0, 0x0aba4, 0x0a5b0, 0x052b0, 0x0b273, 0x06930, 0x07337, 0x06aa0,
	0x0ad50, 0x14b55, 0x04b60, 0x0a570, 0x054e4, 0x0d160, 0x0e968, 0x0d520,
	0x0daa0, 0x16aa6, 0x056d0, 0x04ae0, 0x0a9d4, 0x0a2d0, 0x0d150, 0x0f252,
	0x0d520,
}

func (lc LunarCalendar) LeapDays(y int) int {
	if lc.LeapMonth(y) != 0 {
		if lunarInfo[y-1900]&0x10000 != 0 {
			return 30
		}
		return 29
	}
	return 0
}

func (LunarCalendar) LeapMonth(y int) int {
	return int(lunarInfo[y-1900] & 0xf)
}

func (lc LunarCalendar) LYearDays(y int) int {
	sum := 348
	i := int64(0x8000)
	for i > 0x8 {
		if lunarInfo[y-1900]&i != 0 {
			sum++
		}
		i >>= 1
	}
	return sum + lc.LeapDays(y)
}

func (LunarCalendar) MonthDays(y, m int) int {
	if m > 12 || m < 1 {
		return -1
	}
	if lunarInfo[y-1900]&(0x10000>>m) != 0 {
		return 30
	}
	return 29
}

type LunarResult struct {
	IsToday bool
	LYear   int
	LMonth  int
	LDay    int
	CYear   int
	CMonth  int
	CDay    int
}

func (lc LunarCalendar) SolarToLunar(y, m, d int) (*LunarResult, error) {
	if y < 1900 || y > 2100 {
		return nil, fmt.Errorf("out of range")
	}
	if y == 1900 && m == 1 && d < 31 {
		return nil, fmt.Errorf("out of range")
	}

	dt := time.Date(y, time.Month(m), d, 0, 0, 0, 0, time.UTC)
	base := time.Date(1900, 1, 31, 0, 0, 0, 0, time.UTC)
	offset := int64(dt.Sub(base).Hours() / 24)

	i := 1900
	temp := 0
	for i < 2101 && offset > 0 {
		temp = lc.LYearDays(i)
		offset -= int64(temp)
		i++
	}
	if offset < 0 {
		offset += int64(temp)
		i--
	}

	year := i
	leap := lc.LeapMonth(i)
	isLeap := false

	month := 1
	for month < 13 && offset > 0 {
		if leap > 0 && month == leap+1 && !isLeap {
			month--
			isLeap = true
			temp = lc.LeapDays(year)
		} else {
			temp = lc.MonthDays(year, month)
		}
		if isLeap && month == leap+1 {
			isLeap = false
		}
		offset -= int64(temp)
		month++
	}

	if offset == 0 && leap > 0 && month == leap+1 && !isLeap {
		month--
	}
	if offset < 0 {
		offset += int64(temp)
		month--
	}

	day := int(offset) + 1
	now := time.Now()
	isToday := dt.Year() == now.Year() && dt.Month() == now.Month() && dt.Day() == now.Day()

	return &LunarResult{
		IsToday: isToday,
		LYear:   year,
		LMonth:  month,
		LDay:    day,
		CYear:   y,
		CMonth:  m,
		CDay:    d,
	}, nil
}

if (!self.define) {
  let n,
    s = {};
  const i = (i, e) => (
    (i = new URL(i + '.js', e).href),
    s[i] ||
      new Promise((s) => {
        if ('document' in self) {
          const n = document.createElement('script');
          (n.src = i), (n.onload = s), document.head.appendChild(n);
        } else (n = i), importScripts(i), s();
      }).then(() => {
        let n = s[i];
        if (!n) throw new Error(`Module ${i} didn’t register its module`);
        return n;
      })
  );
  self.define = (e, t) => {
    ...
    s[a] = Promise.all(e.map((n) => r[n] || c(n))).then((n) => (t(...n), o));
  };
}
  ...
  importScripts(),
    ...
    n.precacheAndRoute(
      [
        ...
        {
          url: '/nothing/_next/static/GisEn-iCENAoyM2ClQPnj/_buildManifest.js',
          revision: '6257ed3c4af95fdb14dddee0fadece11',
        },
        {
          url: '/nothing/_next/static/GisEn-iCENAoyM2ClQPnj/_ssgManifest.js',
        ...
        },
        {
          ...
          revision: 'GisEn-iCENAoyM2ClQPnj',
        },
        {

...
export const getPlayers = async (
  ...
  sortedCountries.sort(
    (
      a: { countryCode: string; total: number },
      b: { countryCode: string; total: number }
    ) => (a.total < b.total ? 1 : -1)
  );

  const sortedTitles = titles.map((title) => {
    return {
  ...
  sortedTitles.sort(
    (
      a: { title: ChessTitleAbbreviation; total: number },
      b: { title: ChessTitleAbbreviation; total: number }
    ) => (a.total < b.total ? 1 : -1)
  );

  players.sort((a, b) => {
    const aBlitzBest: number =
      a.stats.find(({ timeClass }) => timeClass === 'blitz')?.best ?? 0;
    const bBlitzBest: number =
      b.stats.find(({ timeClass }) => timeClass === 'blitz')?.best ?? 0;
    return aBlitzBest < bBlitzBest ? 1 : -1;
  });
